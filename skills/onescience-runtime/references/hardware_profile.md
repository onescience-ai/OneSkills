...
本文件用于定义 `onescience-runtime.discover` 输出的“完整硬件画像”结构，以及它与“代码生成交接摘要”的关系。
...
hardware_profile:
  ...
  software:
    ...
    driver_stack:
      ...
      user_space_runtime: dtk
      user_space_delivery: sghpc
      driver_ready: true
  ...
  capabilities:
    ...
    launch_mode: python
    default_node_scope: single_node
...
如需给 runtime selector 或 codegen handoff 提供更明确的运行范围建议，可额外提供：

- `capabilities.default_node_scope`: `single_node` / `multi_node`
...
- `software.driver_stack.user_space_runtime`: DCU 环境应显式标记为 `dtk`
- `software.driver_stack.user_space_delivery`: DCU 环境可标记为 `sghpc`、`module` 或实际平台封装方式
...
- `runtime.discover` 负责识别驱动栈事实
- `installer` 负责消费这些事实做 precheck
- 系统级驱动安装与内核模块管理不属于 `installer` 的职责

对 DCU backend，`user_space_ready=true` 表示 DTK 用户态 runtime 已可用，或封装 DTK 的 `sghpc` 环境已加载并可作为运行上下文。只看到 DCU 设备、`HIP_VISIBLE_DEVICES` 或 `torch import` 不足以单独证明 DCU 运行环境 ready。
