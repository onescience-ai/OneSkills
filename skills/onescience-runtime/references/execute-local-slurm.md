...
进入本执行分支前必须已有当前轮次 preflight 证据（来自 installer 的 `preflight_result`）：`preflight_passed=true`、`execution_readiness=ready`、`evidence.preflight.status=passed`。任一缺失或失败时，不得生成 `run_job.slurm` 或调用 `sbatch`，必须回到 `preflight` 重新委托 `onescience-installer`（`installer_reason=preflight_validation`）执行环境就绪预检。
