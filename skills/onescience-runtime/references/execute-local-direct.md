...
2. 在测试目录生成任务执行脚本，脚本名优先使用 `run_job.sh`；`local_direct` 固定匹配 `./assets/templates/local_direct.sh` 模板，再按 `runtime.conda.enabled` 分支渲染。
...
8. 默认把结果写到当前测试目录的 `logs/`，即 `<work_dir>/logs/`；其中 `job_name` 优先取 `runtime.script.job_name`，否则用测试目录最后一级目录名，但不再作为本地日志目录的额外子目录。
...
EOF
mkdir -p logs
bash run_job.sh > >(tee logs/stdout.log) 2> >(tee logs/stderr.log >&2)
```
