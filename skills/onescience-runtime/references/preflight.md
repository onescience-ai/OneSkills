...
2. 必读字段：
   - `runtime.conda`
   - `runtime.scnet.*`（SCnet 提交任务时必读；若缺失则回退到 `onescience-runsite`，不是 `scnet-chat` 默认值）
   - `runtime.modules.*`
...
配置分工保持两段：`onescience.json` / `runtime.scnet` 的缺失或补齐由 `onescience-runsite` 处理；`runtime.conda` 的缺失或修复由 `onescience-installer` 处理。
