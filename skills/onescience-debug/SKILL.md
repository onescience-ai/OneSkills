...
当你需要对照标准输入结构时，读取 `./assets/examples/`。
...
7. 优先使用当前环境可用的执行方式提交测试；若测试依赖远程环境，先调用 `onescience-hardware` 获取完整硬件画像，再由合适的执行层提交测试；运行类测试优先交给 `onescience-runtime`，环境未就绪时先交给 `onescience-installer`。远程运行结束后的日志应由 `onescience-runtime` 对应通道同步或下载到本地，debug 优先分析本地日志。
8. 输出结构化测试报告；若未成功提交，则先报告提交失败，不推断结果。

当前 debug 链路中，`slurm_dcu` 与 `slurm_gpu` 应视为 `supported`，`slurm_cpu` 应视为 `planned_backend`。不要把 installer 的不支持边界误投射到 debug 链路。
这里也要区分：

- backend support status：当前 debug 链路是否支持该 backend
- precheck outcome：当前 host 上是否已经 ready to debug

## 分层边界

- 本 skill 负责判断“测什么、怎么测、如何汇报”，不负责替代 `onescience-runtime` 做运行提交
- 本 skill 不负责远程日志同步；它消费 `onescience-runtime` 任意执行通道交接的本地日志目录、日志文件列表、`sync_status` 和必要的执行上下文。只有 runtime 未同步且仍提供远端上下文时，才按需读取远端日志。
...
- 即使 backend support status 为 `supported`，只要 precheck outcome 为 `blocked`，也不能继续远程调试
