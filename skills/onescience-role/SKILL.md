...
当上游 `task_method=paper2code`、`domain_task_family=paper-reproduction`，或用户任务明确属于论文复现时，先读取 `./paper2code/paper2code_overview.md` 了解论文复现前处理流水线，再读取 `./paper2code/boundary_contract.md` 作为默认层级约束；当需要把论文复现任务归入更具体的任务桶时，读取 `./paper2code/task_map.md`。论文复现是横向任务方法，不覆盖 `domain_route` 中的真实科学领域；但 role 层不得把它改写为“获取官方代码”的普通模型实现任务，不得选择纯 `model-engineer` 链作为主链。
