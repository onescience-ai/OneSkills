...
- `next_skill`始终为`onescience-skill`，无论`execution_entry`指向`onescience-coder`、`onescience-runtime`还是`onescience-installer`
...
- `next_skill`表示当前立即进入的下一跳，始终为`onescience-skill`；消费方（Codex编排层或opencode路由层）应读取此字段决定下一跳，而非直接跳到`execution_entry`
...
- 不要在角色未明确时直接拉起完整执行链
- 不要跳过交接物定义就把任务下发到执行层
- 不要把`execution_entry`误当成当前立即执行的下一跳；当前立即下一跳始终由`next_skill`指定，固定为`onescience-skill`
