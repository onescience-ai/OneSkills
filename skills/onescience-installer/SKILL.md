...
description: OneScience 环境安装技能。用于根据 onescience.json 安装或验证 OneScience、创建或复用 conda 环境、检测已有 onescience 包、在记录的 conda 环境或当前环境中安装 Python 包、安装 HPC 科学计算软件（如 GROMACS、LAMMPS）；缺少 run_site 或远程 SSH 配置时先调用 onescience-runsite 补齐运行站点配置。
type: executor
---
...
- HPC 软件安装时：`install_intent=hpc_software` 路径不读取 `backend_profiles.json`、`workspace_bootstrap_profiles.json`、`install_domains.json`，也不写回 `runtime.conda`；仅使用 `hpc_software_profiles.json` 和 `install_flow.md` 中 §20-§25 的 HPC 专用模板。
...
- `install_state`
- `verify_state`
- `conda_writeback`（仅 `bootstrap` 和 `python_packages` 路径）
- `software_name`（仅 `hpc_software` 路径）
- `software_version`（仅 `hpc_software` 路径）
- `install_dir`（仅 `hpc_software` 路径）
- `next_action`
- `resume_target`
- `resume_phase`
