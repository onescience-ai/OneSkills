...
#### 方式 1：使用 onescience-runsite 技能（推荐）

如果项目根目录的 `onescience.json` 中缺少 SCNet 配置，调用 `onescience-runsite` 技能配置：


#### 方式 2：手动配置 ~/.scnet-chat.env

首次使用可手动在 `~/.scnet-chat.env` 配置：
...
访问 SCNet 控制台获取访问密钥：https://www.scnet.cn/ui/console/index.html#/personal/auth-manage
