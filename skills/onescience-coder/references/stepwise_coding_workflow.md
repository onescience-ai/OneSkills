...
## 待确认科学决策（逐项必填；没有则写「无」）
- 本步骤将假设的数值（值 + 来源标签）：
  - 例：机柜尺寸 1.2×0.6×1.8 m【literature[6]，proposed_candidate】
  - 无文献依据的值必须写【无依据，纯假设，proposed_candidate】
- 本步骤依赖但尚不存在的输入（及缺失后果）：
  - 例：仿真结果文件（solver 尚未运行）→ 后处理无真实数据可读
- 若上述输入缺失，本步骤诚实状态应为：BLOCKED / PARTIAL(not-a-result)

## 用户操作
请回复：
1. `确认以上假设并执行本步骤`
2. `提供真实值/修改后执行：<数值或修改意见>`
3. `缺输入，先 BLOCKED`
```
...
9. 写入配置/代码的具体数值（尺寸、物性、阈值、网格参数等）必须与门禁「待确认科学决策」清单逐项一致；不得写入门禁未披露的数值。实现中若发现需要新增假设，停止编码并重新输出门禁信息等待确认。
