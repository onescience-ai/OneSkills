...
- **模块族**: fuser
- **统一入口**: not_applicable
- **注册名称**: style="XiheFuser"

## 组件说明
`xihefuse` 属于 `fuser` 模块族，核心实现类/函数包括 `XiheFuser`。它的定位是融合模块，由上层 OneScience 模型或流水线通过 Python API 调用。
...
## 参数
- `XiheFuser` 构造参数：`dim`, `input_resolution`, `num_local`, `num_heads_local`, `num_heads_global`, `window_size`, `mlp_ratio`, `qkv_bias`, `drop_path`, `num_groups`, `num_global`, `depth_local`, `norm_layer`
...
```sh
python -c "from onescience.modules.fuser.xihefuse import XiheFuser; import inspect; print(inspect.signature(XiheFuser))"
```
...
- `XiheFuser` 构造默认参数：`num_heads_local=6`，`num_heads_global=12`，`window_size=(1, 6, 12)`，`mlp_ratio=4.0`，`qkv_bias=True`，`drop_path=0.0`，`num_groups=32`，`num_global=1`，`depth_local=2`，`norm_layer=nn.LayerNorm`

## 运行接口
- `XiheFuser`：实例化后通过 `forward` 等运行时接口参与流水线。
...
### 描述
XiheFuser 是 OneScience 中面向气象/地球系统建模的融合模块，用于在模型流水线中完成特征变换、尺度组织或结果重建。
