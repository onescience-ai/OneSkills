# Contract: XiheLocalSIEFuser

## 基本信息

- **组件名称**: XiheLocalSIEFuser
- **模块族**: fuser
- **统一入口**: not_applicable
- **注册名称**: style="XiheLocalSIEFuser"

## 组件说明

`xihelocalsiefuser` 属于 `fuser` 模块族，核心实现类/函数包括 `XiheLocalSIEFuser`。它的定位是融合模块，由上层 OneScience 模型或流水线通过 Python API 调用。
...
- `XiheLocalSIEFuser` 构造参数：`dim`, `input_resolution`, `depth`, `num_heads`, `window_size`, `mlp_ratio`, `qkv_bias`, `qk_scale`, `drop`, `attn_drop`, `drop_path`, `norm_layer`
