# Contract: MeshGraphDecoder

## 基本信息

- **组件名称**: MeshGraphDecoder
- **模块族**: decoder
- **统一入口**: not_applicable
- **注册名称**: style="MeshGraphDecoder"

## 组件说明

`mesh_graph_decoder` 属于 `decoder` 模块族，核心实现类/函数包括 `MeshGraphDecoder`。它的定位是解码器，由上层 OneScience 模型或流水线通过 Python API 调用。
...
- `MeshGraphDecoder` 构造参数：`aggregation`, `input_dim_src_nodes`, `input_dim_dst_nodes`, `input_dim_edges`, `output_dim_dst_nodes`, `output_dim_edges`, `hidden_dim`, `hidden_layers`, `activation_fn`, `norm_type`, `do_concat_trick`, `recompute_activation`
...
```sh
python -c "from onescience.modules.decoder.mesh_graph_decoder import MeshGraphDecoder; import inspect; print(inspect.signature(MeshGraphDecoder))"
```
...
- `MeshGraphDecoder` 构造默认参数：`aggregation='sum'`，`input_dim_src_nodes=512`，`input_dim_dst_nodes=512`，`input_dim_edges=512`，`output_dim_dst_nodes=512`，`output_dim_edges=512`，`hidden_dim=512`，`hidden_layers=1`，`activation_fn=nn.SiLU()`，`norm_type='LayerNorm'`，`do_concat_trick=False`，`recompute_activation=False`

## 运行接口

- `MeshGraphDecoder`：实例化后通过 `forward` 等运行时接口参与流水线。
...
MeshGraphDecoder 是 OneScience 中面向气象/地球系统建模的解码器，用于在模型流水线中完成特征变换、尺度组织或结果重建。
