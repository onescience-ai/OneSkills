...
- **组件名称**: FourCastNetEmbedding
- **模块族**: embedding
- **统一入口**: not_applicable
- **注册名称**: style="FourCastNetEmbedding"
...
该组件的核心实现包括 `FourCastNetEmbedding`，定位为嵌入层，由上层 OneScience 模型或流水线通过 Python API 调用。
...
```sh
python -c "from onescience.modules.embedding.fourcastnetembedding import FourCastNetEmbedding; import inspect; print(inspect.signature(FourCastNetEmbedding))"
```
...
- `FourCastNetEmbedding` 构造默认参数：`img_size=(720, 1440)`，`patch_size=(8, 8)`，`in_chans=19`，`embed_dim=768`

## 运行接口

- `FourCastNetEmbedding`：实例化后通过 `forward` 等运行时接口参与流水线。
...
FourCastNetEmbedding 是 OneScience 中面向气象/地球系统建模的嵌入层，用于在模型流水线中完成特征变换、尺度组织或结果重建。
