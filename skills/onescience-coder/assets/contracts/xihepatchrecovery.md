# Contract: XihePatchRecovery

## 基本信息

- **组件名称**: XihePatchRecovery
- **模块族**: recovery
- **统一入口**: not_applicable
- **注册名称**: style="XihePatchRecovery"
...
```sh
python -c "from onescience.modules.recovery.xihepatchrecovery import XihePatchRecovery; import inspect; print(inspect.signature(XihePatchRecovery))"
```
...
- `XihePatchRecovery` 构造默认参数：`img_size=(2041, 4320)`，`patch_size=(6, 12)`，`in_chans=192`，`out_chans=96`

## 运行接口
- `XihePatchRecovery`：实例化后通过 `forward` 等运行时接口参与流水线。
...
### 描述
XihePatchRecovery 是 OneScience 中面向气象/地球系统建模的重建模块，用于在模型流水线中完成特征变换、尺度组织或结果重建。
