...
- **模块族**: node
- **统一入口**: not_applicable
- **注册名称**: style="MeshNodeBlock"

## 组件说明

`mesh_node_block` 属于 `node` 模块族，核心实现类/函数包括 `MeshNodeBlock`。它的定位是图节点更新模块，由上层 OneScience 模型或流水线通过 Python API 调用。

## 用途

聚合边消息并更新节点表示，适用于消息传递中的节点更新阶段。
...
- `MeshNodeBlock` 构造参数：`aggregation`, `input_dim_nodes`, `input_dim_edges`, `output_dim`, `hidden_dim`, `hidden_layers`, `activation_fn`, `norm_type`, `recompute_activation`
...
MeshNodeBlock 是 OneScience 中面向气象/地球系统建模的图节点更新模块，用于在模型流水线中完成特征变换、尺度组织或结果重建。
...
- 任务需要 `图节点更新模块` 能力，且组件输入输出契约与当前模型阶段匹配。
