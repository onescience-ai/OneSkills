# Contract: GraphCastEncoderEmbedder

## 基本信息

- **组件名称**: GraphCastEncoderEmbedder
- **模块族**: embedding
- **统一入口**: not_applicable
- **注册名称**: style="GraphCastEncoderEmbedder"
...
```sh
python -c "from onescience.modules.embedding.graphcast_embedder import GraphCastEncoderEmbedder; import inspect; print(inspect.signature(GraphCastEncoderEmbedder))"
```
...
- `GraphCastEncoderEmbedder` 构造默认参数：`input_dim_grid_nodes=474`，`input_dim_mesh_nodes=3`，`input_dim_edges=4`，`output_dim=512`，`hidden_dim=512`，`hidden_layers=1`，`activation_fn=nn.SiLU()`，`norm_type='LayerNorm'`，`recompute_activation=False`
- `GraphCastDecoderEmbedder` 构造默认参数：`input_dim_edges=4`，`output_dim=512`，`hidden_dim=512`，`hidden_layers=1`，`activation_fn=nn.SiLU()`，`norm_type='LayerNorm'`，`recompute_activation=False`

## 运行接口
- `GraphCastEncoderEmbedder`：实例化后通过 `forward` 等运行时接口参与流水线。
- `GraphCastDecoderEmbedder`：实例化后通过 `forward` 等运行时接口参与流水线。
...
### 描述
GraphCast 嵌入组件负责将 grid 节点、mesh 节点、grid-to-mesh 边、mesh 边以及 mesh-to-grid 边特征映射到统一隐空间。
