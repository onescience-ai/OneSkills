# Contract: XiheGlobalSIEFuser

## 基本信息

- **组件名称**: XiheGlobalSIEFuser
- **模块族**: fuser
- **统一入口**: not_applicable
- **注册名称**: style="XiheGlobalSIEFuser"
...
```sh
python -c "from onescience.modules.fuser.xiheglobalsiefuser import XiheGlobalSIEFuser; import inspect; print(inspect.signature(XiheGlobalSIEFuser))"
```
...
- `XiheGlobalSIEFuser` 构造默认参数：`num_heads=12`，`qkv_bias=True`，`num_groups=32`，`norm_layer=nn.LayerNorm`

## 运行接口

- `XiheGlobalSIEFuser`：实例化后通过 `forward` 等运行时接口参与流水线。
...
XiheGlobalSIEFuser 是 OneScience 中面向气象/地球系统建模的融合模块，用于在模型流水线中完成特征变换、尺度组织或结果重建。
