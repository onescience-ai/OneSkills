...
```sh
python -c "import onescience.modules.utils.gnnlayer_utils as m; print([n for n in dir(m) if not n.startswith('_')])"
```
...
- 模块级函数：`checkpoint_identity`、`set_checkpoint_fn`、`concat_message_function`、`concat_efeat_dgl`、`concat_efeat`、`sum_efeat_dgl`、`sum_efeat`、`agg_concat_dgl`、`aggregate_and_concat`。

## 主要函数

- `checkpoint_identity`
- `set_checkpoint_fn`
- `concat_message_function`
- `concat_efeat_dgl`
- `concat_efeat`
- `sum_efeat_dgl`
- `sum_efeat`
- `agg_concat_dgl`
- `aggregate_and_concat`
...
GNN 工具组件提供张量整形、特征拼接、聚合和消息传递辅助函数，服务于 GraphCast 的边更新与节点更新流程。
