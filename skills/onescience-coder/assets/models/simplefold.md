# Model Card: SimpleFold

## 基本信息

- 模型名：`SimpleFold`
- 任务类型：`蛋白质结构预测 / flow matching 生成式折叠`
- 当前状态：`stable`
- 主实现文件：`./onescience/src/onescience/models/simplefold/simplefold.py`
...
1. 先看本模型卡
2. 再看 `simplefold/torch/architecture.py` 或 `simplefold/mlx/architecture.py`
3. 再看 `simplefold/simplefold.py` 中训练、采样和 pLDDT 逻辑
4. 若涉及数据准备，再看 `examples/biosciences/simplefold/process_data.py` 与 `tokenize_data.py`

## 组件契约入口

- `../contracts/simplefoldfoldingdit.md`
- 当前未通过 OneScience `One*` 组件入口拼装，优先按模型源码、组件契约和 example config 对齐
...
- `./onescience/examples/biosciences/simplefold/README.md`
