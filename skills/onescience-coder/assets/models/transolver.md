...
- 多模型对比时，`Transolver` 默认使用点级或 PyG view
- 如果新数据集已经整理成结构网格、规则网格或 `(x, fx)` operator view，应优先查看 `transolver_benchmark.md`，使用 `Transolver (CFD_Benchmark)`，不要直接判定 Transolver 不兼容
- 最小数据协议是 `pos / x / y`
  - `pos`：每个点的 2D 或 3D 坐标
  - `x`：每个点的输入特征，可包含几何、边界条件、来流参数或其它点级物理先验
  - `y`：每个点的目标变量
- 如果 canonical datapipe 返回 `(x, fx, y)` operator view，需要单独生成 Transolver adapter，把坐标放入 `Data.pos`，输入特征放入 `Data.x`
- 如果数据只有规则网格，没有显式点列表，也可以展平为点集，但第一轮规格必须说明展平后的坐标、特征和目标通道
- 如果 README 未明确字段名或目标变量维度，先生成只读探测脚本，不要直接假设 `p`、`u/v` 或 `cp` 等字段名
