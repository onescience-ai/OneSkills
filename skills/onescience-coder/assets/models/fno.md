...
- 新数据集接入，特别是 benchmark / 多模型对比案例，默认使用 `onescience.models.cfd_benchmark.FNO`。
- 推荐写法：`from onescience.models.cfd_benchmark import FNO`，再调用 `FNO.Model(args, device)`。
- 默认不要走 `from onescience.models.cfd_benchmark.model_factory import get_model`，除非明确复刻原 `CFD_Benchmark` runner。
- 模型参数应放在 YAML 配置文件中，训练脚本读取 YAML 后转成 `args` 对象传给 `FNO.Model(args, device)`。
- 不要在默认 benchmark 任务中导入 `onescience.models.pdenneval.fno.FNO1d/FNO2d/FNO3d`，除非用户明确要求复现 `examples/cfd/PDENNEval/FNO`。
- 不要在训练脚本里自写 `FNO1D` 或其它临时 FNO 替代模型。

## 新数据集 benchmark 接入提示

- 多模型对比时，`FNO` 默认使用 operator view：`x / fx / y`
- 最小数据协议是：
  - `x -> (Batch, NumPoints, space_dim)`
  - `fx -> (Batch, NumPoints, fun_dim)` 或 `None`
  - `y -> (Batch, NumPoints, out_dim)`
- 规则网格数据需要同步给出 `shapelist`，并保证 `NumPoints == prod(shapelist)`
- 非结构翼型或点云数据可以走 `geotype="unstructured"` 的 Geo 投影分支，但第一轮规格必须说明 `s1 / s2`、投影分辨率和是否存在坐标归一化
- 如果数据不能稳定 reshape 或投影到规则潜网格，`FNO` 应标记为 `adapter-required` 或 `not-recommended`，不要强行复用训练脚本
