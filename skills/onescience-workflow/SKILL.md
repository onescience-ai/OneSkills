...
- 如果不存在，自动从本 skill 内 `./assets/onescience.json` 模板生成一份到项目根目录 `onescience.json`。
- `./assets/onescience.json` 必须保持与项目级 `onescience.json` 相同的基础字段结构，至少包含 `runtime.backend_id`、`runtime.remote`、`runtime.target`、`runtime.environment`、`runtime.cluster`、`runtime.modules`、`runtime.conda`、`runtime.resources`、`runtime.script`。
