...
ESM 是蛋白质语言模型与结构建模模型集合，覆盖单序列语言建模、MSA 建模、ESMFold 结构预测、GVP inverse folding 和变异效应打分。它在 OneScience 中不是单一网络，而是一组蛋白序列基础模型：
...
- `esm_lm_contact_heads`
- `esm_position_embeddings`
- `esm_axial_attention`
- `esm_multihead_attention`
- `esm_rotary_embedding`
