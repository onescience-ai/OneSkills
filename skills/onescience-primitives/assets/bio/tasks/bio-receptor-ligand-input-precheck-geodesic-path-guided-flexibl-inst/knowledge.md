...
- 场景: bio-geodesic-path-guided-flexible-molecular-docking-scenario (B49)
- step_id: s01
- depend: []
...
## 本实例输入槽
- {DOCKING_INPUT} | required=True | type=doc | var_name=对接输入 | hint=输入复合物或配体数据 | default=flexible_ligands.sdf
- {MODEL_NAME} | required=True | type=enum | var_name=对接模型 | hint=选择场景使用的模型 | default=GeoDirDock
