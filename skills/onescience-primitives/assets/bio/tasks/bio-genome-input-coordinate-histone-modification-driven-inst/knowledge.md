# 实例任务：基因组输入与坐标规范化 @ B68

- domain: bio
- 骨架: bio-genome-input-coordinate-normalization-task
- 场景: bio-histone-modification-driven-gene-expression-prediction-scenario (B68)
- step_id: s01
- depend: []
...
## 本实例输入槽
- {GENOMICS_INPUT} | required=True | type=doc | var_name=基因组输入 | hint=输入序列区间或变异 | default=histone_bins.csv
- {MODEL_NAME} | required=True | type=enum | var_name=基因组模型 | hint=选择场景使用的模型 | default=DeepChrome
