# 实例任务：宏基因组输入与任务定义 @ B91

- domain: bio
- 骨架: bio-metagenome-input-task-definition-task
- 场景: bio-phage-and-prokaryotic-host-interaction-prediction-scenario (B91)
- step_id: s01
- depend: []
...
## 本实例输入槽
- {METAGENOME_INPUT} | required=True | type=doc | var_name=宏基因组输入 | hint=输入序列或特征表 | default=phage_host_graph.csv
- {MODEL_NAME} | required=True | type=enum | var_name=宏基因组模型 | hint=选择场景使用的模型 | default=CHERRY
