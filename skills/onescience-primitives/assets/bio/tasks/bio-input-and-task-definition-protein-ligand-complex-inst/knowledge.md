...
- 场景: bio-protein-ligand-complex-structure-and-binding-affinity-joint-scenario (B05)
- step_id: s01
- depend: []
...
## 本实例输入槽
- {INPUT_DATA} | required=True | type=doc | var_name=序列或结构输入 | hint=输入FASTA或结构文件 | default=TYK2_complex.yaml
- {MODEL_NAME} | required=True | type=enum | var_name=结构模型 | hint=选择场景使用的模型 | default=Boltz
