...
## description
为 GenScore 打分与虚筛数据集合 选择接入、解析、分流和评测路线的规划知识。

## when_to_use
当任务与该数据源直接相关，且需要先判断它是训练样本、推理输入、benchmark 分割还是数据库依赖时使用。

## inputs
数据根目录：/public/share/sugonhpcapp01/onestore/onedatasets/GenScore/genscore_data。任务输入通常是序列、结构、图、图像、问答对、标签、数据库包或预处理缓存。
