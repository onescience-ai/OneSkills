...
基于归因分析任务（任务ID：334，MOF合成应用多模态机器学习关联）的JSON Schema报告交付契约验证案例：

#### JSON Schema报告交付契约验证案例
...
| 参数 | 值 | 来源 | 说明 |
|------|-----|------|------|
| 必填字段 | issues, summary, task, task_id | 任务334案例 | 归因报告的顶层必填字段 |
| 额外字段 | error, sessionID, timestamp, type | 任务334案例 | 不允许的额外字段 |
| task_id类型 | 字符串 | 任务334案例 | 必须与任务索引一致 |
| task类型 | 字符串 | 任务334案例 | 必须与任务name一致 |
| summary类型 | 非空字符串 | 任务334案例 | 不允许空字符串 |
| issues类型 | 数组 | 任务334案例 | 必须是数组类型 |

## 批次补充（2026-09-17：任务70案例补充）

### 实际应用案例

基于归因分析任务（任务ID：70，深度突变扫描监督的蛋白变异效应预测）的JSON Schema报告交付契约验证案例：

#### JSON Schema报告交付契约验证案例
- **契约要求**：report.json必须包含必填字段 ['issues', 'summary', 'task', 'task_id']，且task_id与任务索引一致，task与任务name一致
- **验证失败**：CLI退出码为1；报告校验错误
- **错误详情**：缺少顶层字段 ['issues', 'summary', 'task', 'task_id']；包含额外顶层字段 ['error', 'sessionID', 'timestamp', 'type']；task_id与任务索引不一致；task与任务name不一致；summary必须是非空字符串；issues必须是数组
- **修复方法**：在输出前按契约校验报告，使用report-schema.json校验最终输出并执行错配字段反例测试；确保CLI进程正常退出并返回符合Schema的JSON

### 校准数值（案例专属值，供量级校准）

| 参数 | 值 | 来源 | 说明 |
|------|-----|------|------|
| 必填字段 | issues, summary, task, task_id | 任务70案例 | 归因报告的顶层必填字段 |
| 额外字段 | error, sessionID, timestamp, type | 任务70案例 | 不允许的额外字段 |
| task_id类型 | 字符串 | 任务70案例 | 必须与任务索引一致 |
| task类型 | 字符串 | 任务70案例 | 必须与任务name一致 |
| summary类型 | 非空字符串 | 任务70案例 | 不允许空字符串 |
| issues类型 | 数组 | 任务70案例 | 必须是数组类型 |
| CLI退出码 | 1 | 任务70案例 | 非零退出码表示执行失败 |

## 批次补充（2026-09-21：任务305案例补充）

### 实际应用案例

基于归因分析任务（任务ID：305，近地面臭氧小时级多时效预报）的JSON Schema报告交付契约验证案例：
...
| 参数 | 值 | 来源 | 说明 |
|------|-----|------|------|
| 必填字段 | issues, summary, task, task_id | 任务305案例 | 归因报告的顶层必填字段 |
| 额外字段 | error, sessionID, timestamp, type | 任务305案例 | 不允许的额外字段 |
| task_id类型 | 字符串 | 任务305案例 | 必须与任务索引一致 |
| task类型 | 字符串 | 任务305案例 | 必须与任务name一致 |
| summary类型 | 非空字符串 | 任务305案例 | 不允许空字符串 |
| issues类型 | 数组 | 任务305案例 | 必须是数组类型 |
| CLI退出码 | 1 | 任务305案例 | 非零退出码表示执行失败 |
