...
## 批次补充（2026-09-17：任务51归因分析报告契约案例）

### 实际应用案例

基于归因分析任务（任务ID：51，大卷积核RNA二级结构预测）的报告契约案例：
...
#### 与前次失败案例的对比
- Task 29、Task 49、Task 81、Task 291、Task 292同样出现退出码1和结构化输出契约失败
- Task 51的特征与Task 292相似：缺少必需字段、包含额外字段、任务身份不一致
- 共同根因：CLI执行成功但输出格式不符合Schema校验要求
...
| 参数 | 值 | 来源 | 说明 |
|------|-----|------|------|
| 必填字段数量 | 4 | 任务51案例 | task_id, task, summary, issues |
| 禁止额外字段数量 | 4 | 任务51案例 | error, sessionID, timestamp, type |
| 任务身份验证点 | 2 | 任务51案例 | task_id与索引一致，task与name一致 |
| 字段类型约束 | 2 | 任务51案例 | summary为非空字符串，issues为数组 |

## 批次补充（2026-09-18：任务277归因分析报告契约案例）

### 实际应用案例

基于归因分析任务（任务ID：277，有限气象变量驱动的未来7天参考蒸散发预报）的报告契约案例：
...
#### 与前次失败案例的对比
- Task 29、Task 49、Task 51、Task 81、Task 283、Task 291、Task 292同样出现退出码1和结构化输出契约失败
- Task 277的特征与Task 51相似：缺少必需字段、包含额外字段、任务身份不一致
- 共同根因：CLI执行成功但输出格式不符合Schema校验要求
...
| 参数 | 值 | 来源 | 说明 |
|------|-----|------|------|
| 必填字段数量 | 4 | 任务277案例 | task_id, task, summary, issues |
| 禁止额外字段数量 | 4 | 任务277案例 | error, sessionID, timestamp, type |
| 任务身份验证点 | 2 | 任务277案例 | task_id与索引一致，task与name一致 |
| 字段类型约束 | 2 | 任务277案例 | summary为非空字符串，issues为数组 |