...
Use this primitive when the task explicitly involves `iso-standards-readiness`, its scientific workflow, or its associated artifacts and data structures.
...
Pass `primitive_id: general.application.iso_standards_readiness`, source references used, package/version assumptions, input artifacts, output expectations, and unresolved validation risks to the next workflow step.
