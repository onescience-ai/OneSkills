...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/U_NO.py`

## 模型架构概览

U_NO 的架构定位是 `U-shaped Neural Operator`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：将 U-Net 下采样/上采样结构与 Fourier operator 处理结合，在多尺度路径中注入谱算子建模能力。
...
- 主干结构：OneMlp 预处理 + U-Net 编码解码层 + 各尺度 OneFourier 处理 + Linear 输出头。
...
主干计算
  结构分支补齐到 16 的倍数后进入多尺度 U 型路径，各尺度谱处理后裁剪并展平输出。
...
from onescience.models.cfd_benchmark import U_NO
import torch
...
model = U_NO.Model(args, device).to(device)
...
- 支持范围：需要比 FNO 更强的多尺度表达、同时保持算子学习属性的规则网格 CFD 任务。
- 限制条件：结构尺寸过小导致多次下采样不稳定，或无法提供正确 `shapelist` 的任务。
...
U_NO 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：将 U-Net 下采样/上采样结构与 Fourier operator 处理结合，在多尺度路径中注入谱算子建模能力。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：需要比 FNO 更强的多尺度表达、同时保持算子学习属性的规则网格 CFD 任务。。
- 当用户要求复用 `onescience.models.cfd_benchmark.U_NO` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `task`, `time_input`, `unified_pos`。
- 不建议使用场景：结构尺寸过小导致多次下采样不稳定，或无法提供正确 `shapelist` 的任务。。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/U_NO.py`
