...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/GNOT.py`

## 模型架构概览

GNOT 的架构定位是 `graph neural operator transformer`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：分别预处理坐标分支和场特征分支，再通过 GNOT block 建模位置与函数值之间的算子交互。
...
- 主干结构：preprocess_x + preprocess_z + 多层 OneTransformer(GNOT_block)，末层输出目标场。
- 核心业务入口：`forward`。
...
主干计算
  x 与 fx 分别映射到隐藏维，block 交互后输出 `(Batch, NumPoints, out_dim)`。
...
from onescience.models.cfd_benchmark import GNOT
import torch
...
model = GNOT.Model(args, device).to(device)
...
- 支持范围：坐标与输入场需要双分支交互建模的点级/网格级算子任务。
- 限制条件：只有单一路输入且无法构造有信息量 fx 的任务；GNOT 的双分支优势会被削弱。
...
GNOT 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：分别预处理坐标分支和场特征分支，再通过 GNOT block 建模位置与函数值之间的算子交互。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：坐标与输入场需要双分支交互建模的点级/网格级算子任务。。
- 当用户要求复用 `onescience.models.cfd_benchmark.GNOT` 或进行 CFD_Benchmark 多模型对比。
...
- 不建议使用场景：只有单一路输入且无法构造有信息量 fx 的任务；GNOT 的双分支优势会被削弱。。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/GNOT.py`
