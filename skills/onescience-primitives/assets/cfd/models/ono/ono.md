...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/ONO.py`

## 模型架构概览

ONO 的架构定位是 `orthogonal neural operator`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：用坐标分支、场特征分支和 ONO block 构造正交/低秩风格算子近似，末层 block 直接输出目标变量。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `attn_type`, `dropout`, `fun_dim`, `geotype`, `mlp_ratio`, `n_heads`, `n_hidden`, `n_layers`, `out_dim`, `psi_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- 主干结构：preprocess_x + preprocess_z + 多层 OneTransformer(ONO_block)，由 `psi_dim/attn_type` 控制算子近似方式。
- 核心业务入口：`forward`。
...
主干计算
  x 和 fx 预处理后保持 token 形态，ONO block 链路最终输出 `(Batch, NumPoints, out_dim)`。
...
from onescience.models.cfd_benchmark import ONO
import torch
...
model = ONO.Model(args, device).to(device)
...
- 支持范围：需要比较 nystrom/linear/selfAttention 等算子注意力近似的点级场预测。
- 限制条件：缺少双分支输入或 `psi_dim/attn_type` 未配置的任务。
...
ONO 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：用坐标分支、场特征分支和 ONO block 构造正交/低秩风格算子近似，末层 block 直接输出目标变量。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：需要比较 nystrom/linear/selfAttention 等算子注意力近似的点级场预测。。
- 当用户要求复用 `onescience.models.cfd_benchmark.ONO` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `attn_type`, `dropout`, `fun_dim`, `geotype`, `mlp_ratio`, `n_heads`, `n_hidden`, `n_layers`, `out_dim`, `psi_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
- 不建议使用场景：缺少双分支输入或 `psi_dim/attn_type` 未配置的任务。。
...
- 模型超参：源码读取字段 `act`, `attn_type`, `dropout`, `fun_dim`, `geotype`, `mlp_ratio`, `n_heads`, `n_hidden`, `n_layers`, `out_dim`, `psi_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/ONO.py`
