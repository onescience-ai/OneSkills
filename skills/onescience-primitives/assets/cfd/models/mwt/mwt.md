...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/MWT.py`

## 模型架构概览

MWT 的架构定位是 `multiwavelet transform operator`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：使用多小波变换式 Fourier 组件在多尺度频域中建模物理场，结构分支会把空间尺寸补到 2 的幂。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `fun_dim`, `geotype`, `modes`, `mwt_k`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- 主干结构：OneMlp 预处理 + 多层 OneFourier(MWT) + Linear 输出；非结构分支经 Geo 投影。
...
主干计算
  输入升维后进入网格形态，补齐到 2 的幂后做 MWT 主干，最终裁剪/展平回点级输出。
...
MWT 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：使用多小波变换式 Fourier 组件在多尺度频域中建模物理场，结构分支会把空间尺寸补到 2 的幂。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：规则网格上需要多尺度频域表达、且可接受 2 的幂 padding 的算子学习任务。。
- 当用户要求复用 `onescience.models.cfd_benchmark.MWT` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `geotype`, `modes`, `mwt_k`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
- 不建议使用场景：`shapelist` 与真实网格不一致或 padding 后边界处理敏感的任务。。
...
- 模型超参：源码读取字段 `act`, `fun_dim`, `geotype`, `modes`, `mwt_k`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/MWT.py`
