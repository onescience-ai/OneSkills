...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/PointNet.py`

## 模型架构概览

PointNet 的架构定位是 `点云 MLP 基线`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：将点坐标与点级特征拼接后用共享 MLP 做局部点表示和输出，是不显式使用邻接关系的轻量点云基线。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `fun_dim`, `n_hidden`, `out_dim`, `space_dim`。
...
- 主干结构：OneMlp encoder/feature extractor + pointwise decoder。
- 核心业务入口：`forward`。
...
主干计算
  常将 batch=1 输入 squeeze 成点集矩阵处理，输出再恢复为 `(1, NumPoints, out_dim)`。
...
from onescience.models.cfd_benchmark import PointNet
import torch
...
model = PointNet.Model(args, device).to(device)
...
- 支持范围：需要快速建立非结构点集回归下限，或图拓扑暂不可用的点云 CFD 任务。
- 限制条件：强依赖局部邻域、边界连通性或长程相互作用的任务。
...
PointNet 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：将点坐标与点级特征拼接后用共享 MLP 做局部点表示和输出，是不显式使用邻接关系的轻量点云基线。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：需要快速建立非结构点集回归下限，或图拓扑暂不可用的点云 CFD 任务。。
- 当用户要求复用 `onescience.models.cfd_benchmark.PointNet` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `n_hidden`, `out_dim`, `space_dim`。
- 不建议使用场景：强依赖局部邻域、边界连通性或长程相互作用的任务。。
...
- 模型超参：源码读取字段 `act`, `fun_dim`, `n_hidden`, `out_dim`, `space_dim`。
...
- ../contracts/torch_geometric.md

## 源码锚点

- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/PointNet.py`
