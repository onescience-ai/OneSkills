...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/GraphSAGE.py`

## 模型架构概览

GraphSAGE 的架构定位是 `图邻域聚合`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：将坐标与节点特征拼接编码后，在给定 edge_index 上使用 SAGEConv 做多层邻域聚合并解码为节点物理量。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `fun_dim`, `n_hidden`, `n_layers`, `out_dim`, `space_dim`。
...
- 主干结构：OneMlp encoder + SAGEConv 输入层/隐藏层/输出层 + BatchNorm + OneMlp decoder。
- 核心业务入口：`forward`。
...
主干计算
  源码会把 batch=1 的 `(1, N, C)` squeeze 为 `(N, C)`，输出再 `unsqueeze(0)` 成 `(1, N, out_dim)`。
...
GraphSAGE 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：将坐标与节点特征拼接编码后，在给定 edge_index 上使用 SAGEConv 做多层邻域聚合并解码为节点物理量。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：已有稳定图拓扑且希望使用轻量图基线的非结构网格节点回归。。
- 当用户要求复用 `onescience.models.cfd_benchmark.GraphSAGE` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `n_hidden`, `n_layers`, `out_dim`, `space_dim`。
- 不建议使用场景：没有 `edge_index` 的数据，或需要复杂多尺度池化/解池化的图任务。。
...
- 模型超参：源码读取字段 `act`, `fun_dim`, `n_hidden`, `n_layers`, `out_dim`, `space_dim`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/GraphSAGE.py`
