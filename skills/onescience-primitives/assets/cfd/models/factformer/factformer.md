...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Factformer.py`
...
from onescience.models.cfd_benchmark import Factformer
import torch
...
model = Factformer.Model(args, device).to(device)
...
- 支持范围：结构化网格上的中大规模场预测，尤其需要比普通自注意力更省的全局交互。
- 限制条件：非结构化几何；源码会对 `geotype='unstructured'` 抛出异常。
...
Factformer 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：将坐标/位置编码与场特征拼接后映射为 token，通过多层 Factformer_block 做结构化网格全局建模。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：结构化网格上的中大规模场预测，尤其需要比普通自注意力更省的全局交互。。
- 当用户要求复用 `onescience.models.cfd_benchmark.Factformer` 或进行 CFD_Benchmark 多模型对比。
...
- 不建议使用场景：非结构化几何；源码会对 `geotype='unstructured'` 抛出异常。。
