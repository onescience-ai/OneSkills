...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/RegDGCNN.py`

## 模型架构概览

RegDGCNN 的架构定位是 `动态图卷积点云回归`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：基于点特征动态构图并提取局部 EdgeConv 风格特征，适合点云/非结构网格的节点级回归。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`dropout`, `emb_dims`, `fun_dim`, `n_hidden`, `out_dim`, `space_dim`。
...
- 主干结构：动态图 kNN 特征构造 + 多层卷积/BatchNorm/Dropout + embedding + 回归输出头。
- 核心业务入口：`forward`。
...
主干计算
  坐标与 fx 拼接为点特征，经动态图卷积得到每点 embedding，输出 `(Batch, NumPoints, out_dim)`。
...
RegDGCNN 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：基于点特征动态构图并提取局部 EdgeConv 风格特征，适合点云/非结构网格的节点级回归。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：点云中局部几何邻域很重要，但没有固定图边或希望动态邻域建模的任务。。
- 当用户要求复用 `onescience.models.cfd_benchmark.RegDGCNN` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`dropout`, `emb_dims`, `fun_dim`, `n_hidden`, `out_dim`, `space_dim`。
- 不建议使用场景：期望使用外部 `geo` 图结构的任务；当前源码忽略 `geo`，由特征动态构图。。
...
- 模型超参：源码读取字段 `dropout`, `emb_dims`, `fun_dim`, `n_hidden`, `out_dim`, `space_dim`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/RegDGCNN.py`
