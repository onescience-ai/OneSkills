...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/U_FNO.py`

## 模型架构概览

U_FNO 的架构定位是 `U-Net enhanced Fourier operator`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：在 FNO 谱卷积主干旁加入 U_Net 多尺度分支，以同时捕获全局频域关系和局部多尺度结构。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- 主干结构：OneMlp 预处理 + 4 层 Fourier/1x1 残差 + 并联 U_Net.multiscale 分支 + Linear 输出。
...
主干计算
  结构分支重排为 `(Batch, n_hidden, *shapelist)`，每层结合谱卷积、点卷积和多尺度 U-Net，再展平输出。
...
from onescience.models.cfd_benchmark import U_FNO
import torch
...
model = U_FNO.Model(args, device).to(device)
...
- 支持范围：规则网格场预测中既需要 FNO 全局建模又希望加强局部细节的对比实验。
- 限制条件：无法稳定形成规则网格的点云；非结构分支仍依赖 2D Geo 投影。
...
U_FNO 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：在 FNO 谱卷积主干旁加入 U_Net 多尺度分支，以同时捕获全局频域关系和局部多尺度结构。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：规则网格场预测中既需要 FNO 全局建模又希望加强局部细节的对比实验。。
- 当用户要求复用 `onescience.models.cfd_benchmark.U_FNO` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
- 不建议使用场景：无法稳定形成规则网格的点云；非结构分支仍依赖 2D Geo 投影。。
...
- 模型超参：源码读取字段 `act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/U_FNO.py`
