...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Galerkin_Transformer.py`

## 模型架构概览

Galerkin_Transformer 的架构定位是 `Galerkin attention Transformer`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：将场特征编码为 token 后通过 Galerkin 风格注意力 block 学习全局算子关系，末层直接映射目标变量。
...
- 主干结构：OneMlp 预处理 + 多层 OneTransformer(GalerkinTransformer_block)，最后一层输出 `out_dim`。
- 核心业务入口：`forward`。
...
主干计算
  输入拼接后映射到 `(Batch, NumPoints, n_hidden)`，block 链路保持 token 数，末层输出 `(Batch, NumPoints, out_dim)`。
...
from onescience.models.cfd_benchmark import Galerkin_Transformer
import torch
...
model = Galerkin_Transformer.Model(args, device).to(device)
...
- 支持范围：需要全局 token 交互但希望采用 Galerkin attention 近似的结构化或点级算子基线。
- 限制条件：点数极大且没有降采样策略的任务；注意力类主干显存会随 token 数增长。
...
Galerkin_Transformer 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：将场特征编码为 token 后通过 Galerkin 风格注意力 block 学习全局算子关系，末层直接映射目标变量。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：需要全局 token 交互但希望采用 Galerkin attention 近似的结构化或点级算子基线。。
- 当用户要求复用 `onescience.models.cfd_benchmark.Galerkin_Transformer` 或进行 CFD_Benchmark 多模型对比。
...
- 不建议使用场景：点数极大且没有降采样策略的任务；注意力类主干显存会随 token 数增长。。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Galerkin_Transformer.py`
