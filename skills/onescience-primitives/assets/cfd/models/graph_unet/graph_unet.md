...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Graph_UNet.py`
...
from onescience.models.cfd_benchmark import Graph_UNet
import torch
...
model = Graph_UNet.Model(args, device).to(device)
...
- 支持范围：非结构图中存在明显多尺度局部结构，且图拓扑可以稳定构造的节点回归任务。
- 限制条件：点序或图连接不稳定的任务；池化会改变节点集合，需要谨慎维护索引映射。
...
Graph_UNet 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：在图结构上通过 GCN/TopKPooling/上采样形成编码-解码路径，用多尺度图表示输出节点级物理场。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：非结构图中存在明显多尺度局部结构，且图拓扑可以稳定构造的节点回归任务。。
- 当用户要求复用 `onescience.models.cfd_benchmark.Graph_UNet` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `n_hidden`, `out_dim`。
- 不建议使用场景：点序或图连接不稳定的任务；池化会改变节点集合，需要谨慎维护索引映射。。
...
- 模型超参：源码读取字段 `act`, `fun_dim`, `n_hidden`, `out_dim`。
