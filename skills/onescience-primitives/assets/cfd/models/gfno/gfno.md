...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/GFNO.py`

## 模型架构概览

GFNO 的架构定位是 `group equivariant Fourier operator`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：在 FNO 思路上加入群等变谱卷积与等变 MLP，用于利用二维场中的旋转/反射等对称先验。
...
- 主干结构：OneMlp 预处理 + GFNO stem + OneEquivariant/GroupEquivariantMLP2d 输出；非结构分支使用 Geo 投影。
- 核心业务入口：`_gfno_stem_forward`, `structured_geo`, `unstructured_geo`, `forward`。
- 源码类：`Model`；若存在辅助类，则包括 `GNorm, Model`。
...
主干计算
  结构分支进入二维网格特征，等变谱主干后再展平；非结构分支先投影到默认潜在网格再返回点级输出。
...
from onescience.models.cfd_benchmark import GFNO
import torch
...
model = GFNO.Model(args, device).to(device)
...
- 支持范围：具有明显几何对称性、希望比较等变先验收益的 2D CFD 场预测。
- 限制条件：非 2D 场、无对称先验或 `space_dim/shapelist` 与 GFNO 设定不匹配的任务。
...
GFNO 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：在 FNO 思路上加入群等变谱卷积与等变 MLP，用于利用二维场中的旋转/反射等对称先验。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：具有明显几何对称性、希望比较等变先验收益的 2D CFD 场预测。。
- 当用户要求复用 `onescience.models.cfd_benchmark.GFNO` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
- 不建议使用场景：非 2D 场、无对称先验或 `space_dim/shapelist` 与 GFNO 设定不匹配的任务。。
