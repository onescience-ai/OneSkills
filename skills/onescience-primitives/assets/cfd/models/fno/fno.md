...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/FNO.py`

## 模型架构概览

FNO 的架构定位是 `Fourier Neural Operator`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：先用 MLP 做点级升维，再在 1D/2D/3D 规则网格或 Geo 投影网格上使用谱卷积进行全局场到场建模。
...
- 主干结构：OneMlp 预处理 + 4 层 OneFourier(FNOSpectralConv*d) 与 1x1 卷积残差 + Linear 输出头。
...
主干计算
  结构分支从 `(Batch, NumPoints, n_hidden)` 重排为 `(Batch, n_hidden, *shapelist)`，谱处理后再展平回点级输出。
...
from onescience.models.cfd_benchmark import FNO
import torch
...
model = FNO.Model(args, device).to(device)
...
- 支持范围：规则网格稳态/动态场预测和需要强全局低频建模的基线任务。
- 限制条件：无法稳定 reshape 为 `shapelist` 的点云，或高频局部细节极强且不适合谱截断的任务。
...
FNO 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：先用 MLP 做点级升维，再在 1D/2D/3D 规则网格或 Geo 投影网格上使用谱卷积进行全局场到场建模。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：规则网格稳态/动态场预测和需要强全局低频建模的基线任务。。
- 当用户要求复用 `onescience.models.cfd_benchmark.FNO` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
- 不建议使用场景：无法稳定 reshape 为 `shapelist` 的点云，或高频局部细节极强且不适合谱截断的任务。。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/FNO.py`
