...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Transolver.py`

## 模型架构概览

Transolver 的架构定位是 `物理切片 Transformer`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：CFD_Benchmark 版本使用标准 `(x, fx, T)` token 接口，通过 Transolver_block 的物理状态切片注意力进行全局建模。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `dropout`, `fun_dim`, `geotype`, `mlp_ratio`, `n_heads`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `slice_num`, `space_dim`, `time_input`, `unified_pos`。
...
- 主干结构：OneMlp 预处理 + 多层 OneTransformer(Transolver_block, slice_num)；末层 block 输出 `out_dim`。
...
主干计算
  x/fx 拼接预处理为 `(Batch, NumPoints, n_hidden)`，切片注意力保持 token 形态并输出点级目标。
...
from onescience.models.cfd_benchmark import Transolver
import torch
...
model = Transolver.Model(args, device).to(device)
...
- 支持范围：非结构或结构 token 都可组织成 `(x, fx)` 的 CFD 场预测，尤其需要几何感知全局交互时。
- 限制条件：误把该文件当成 PyG Data 接口的 Transolver2D/3D；两者输入协议不同。
...
Transolver 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：CFD_Benchmark 版本使用标准 `(x, fx, T)` token 接口，通过 Transolver_block 的物理状态切片注意力进行全局建模。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：非结构或结构 token 都可组织成 `(x, fx)` 的 CFD 场预测，尤其需要几何感知全局交互时。。
- 当用户要求复用 `onescience.models.cfd_benchmark.Transolver` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `dropout`, `fun_dim`, `geotype`, `mlp_ratio`, `n_heads`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `slice_num`, `space_dim`, `time_input`, `unified_pos`。
- 不建议使用场景：误把该文件当成 PyG Data 接口的 Transolver2D/3D；两者输入协议不同。。
...
- 模型超参：源码读取字段 `act`, `dropout`, `fun_dim`, `geotype`, `mlp_ratio`, `n_heads`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `slice_num`, `space_dim`, `time_input`, `unified_pos`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Transolver.py`
