...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/LSM.py`

## 模型架构概览

LSM 的架构定位是 `latent spectral model`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：将输入场映射到隐藏空间后结合谱卷积、Transformer 和 U 型多尺度结构，在潜在空间中学习 CFD 算子。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `fun_dim`, `geotype`, `modes`, `n_heads`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `task`, `time_input`, `unified_pos`。
...
- 主干结构：OneMlp 预处理 + 谱/Transformer 混合主干 + U-shape 多尺度卷积 + Linear 输出。
...
主干计算
  结构分支重排为网格特征，非结构分支经 Geo 投影进入潜在规则网格，再恢复点级输出。
...
from onescience.models.cfd_benchmark import LSM
import torch
...
model = LSM.Model(args, device).to(device)
...
- 支持范围：希望同时比较谱算子、注意力和多尺度局部结构的强基线任务。
- 限制条件：资源很紧或只需要轻量基线的任务；LSM 组件多，调参和排错成本更高。
...
LSM 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：将输入场映射到隐藏空间后结合谱卷积、Transformer 和 U 型多尺度结构，在潜在空间中学习 CFD 算子。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：希望同时比较谱算子、注意力和多尺度局部结构的强基线任务。。
- 当用户要求复用 `onescience.models.cfd_benchmark.LSM` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `geotype`, `modes`, `n_heads`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `task`, `time_input`, `unified_pos`。
- 不建议使用场景：资源很紧或只需要轻量基线的任务；LSM 组件多，调参和排错成本更高。。
...
- 模型超参：源码读取字段 `act`, `fun_dim`, `geotype`, `modes`, `n_heads`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `task`, `time_input`, `unified_pos`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/LSM.py`
