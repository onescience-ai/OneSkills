...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/U_Net.py`

## 模型架构概览

U_Net 的架构定位是 `CFD_Benchmark U-shape field surrogate`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：CFD_Benchmark 版本用统一 `(x, fx)` 接口进入 U-shape 多尺度编码解码主干，与 DeepCFD 图像 UNet 不是同一套接口。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `task`, `time_input`, `unified_pos`。
...
- 主干结构：OneMlp 预处理 + OneEncoder + OneDecoder + OneHead + Linear 输出；非结构分支可经 GeoSpectralConv2d 投影。
- 核心业务入口：`multiscale`, `structured_geo`, `unstructured_geo`, `forward`。
...
主干计算
  点级 token 重排为网格特征，经 U-shape 主干后展平为 `(Batch, NumPoints, out_dim)`。
...
from onescience.models.cfd_benchmark import U_Net
import torch
...
model = U_Net.Model(args, device).to(device)
...
- 支持范围：可稳定 reshape 为规则网格的 CFD 场代理建模，或需要和 FNO/U_FNO/U_NO 使用同一 benchmark datapipe 对比。
- 限制条件：直接喂 `(Batch, Channels, H, W)` 图像 batch，或把非结构点云硬 reshape 成网格。
...
U_Net 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：CFD_Benchmark 版本用统一 `(x, fx)` 接口进入 U-shape 多尺度编码解码主干，与 DeepCFD 图像 UNet 不是同一套接口。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：可稳定 reshape 为规则网格的 CFD 场代理建模，或需要和 FNO/U_FNO/U_NO 使用同一 benchmark datapipe 对比。。
- 当用户要求复用 `onescience.models.cfd_benchmark.U_Net` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `task`, `time_input`, `unified_pos`。
- 不建议使用场景：直接喂 `(Batch, Channels, H, W)` 图像 batch，或把非结构点云硬 reshape 成网格。。
...
- 模型超参：源码读取字段 `act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `out_dim`, `ref`, `shapelist`, `space_dim`, `task`, `time_input`, `unified_pos`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/U_Net.py`
