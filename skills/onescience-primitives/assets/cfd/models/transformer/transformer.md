...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Transformer.py`

## 模型架构概览

Transformer 的架构定位是 `标准自注意力 Transformer`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：将坐标与场特征编码为 token 后使用多层自注意力 block 做全局交互，末层直接输出目标场。
...
- 主干结构：OneMlp 预处理 + 多层 Transformer_block(OneAttention + MLP)，最后一层输出 `out_dim`。
- 核心业务入口：`forward`。
- 源码类：`Model`；若存在辅助类，则包括 `Transformer_block, Model`。
...
主干计算
  输入拼接后为 `(Batch, NumPoints, n_hidden)`，注意力 block 保持 token 数，输出 `(Batch, NumPoints, out_dim)`。
...
from onescience.models.cfd_benchmark import Transformer
import torch
...
model = Transformer.Model(args, device).to(device)
...
- 支持范围：需要普通全局自注意力基线、点数适中且希望接口与其他 CFD_Benchmark 模型一致的任务。
- 限制条件：NumPoints 很大且没有稀疏化/降采样策略的任务。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Transformer.py`
