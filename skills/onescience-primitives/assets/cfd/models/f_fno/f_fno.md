...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/F_FNO.py`

## 模型架构概览

F_FNO 的架构定位是 `factorized Fourier operator`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：用分解式 Fourier 卷积沿各空间轴建模，相比标准 FNO 更强调各维度频域因子的高效组合。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- 主干结构：OneMlp 预处理 + `n_layers` 层 FFNO 谱卷积 + Linear 输出头；非结构分支经 GeoSpectralConv2d/IPHI 投影。
...
主干计算
  结构分支重排到 `(Batch, n_hidden, *shapelist)`，每层按空间维做谱卷积，再恢复为 `(Batch, NumPoints, out_dim)`。
...
from onescience.models.cfd_benchmark import F_FNO
import torch
...
model = F_FNO.Model(args, device).to(device)
...
- 支持范围：规则网格多维场预测，希望在 FNO 基础上控制谱卷积计算量的对比实验。
- 限制条件：点集无法合理投影到规则潜在网格，或 `shapelist` 与真实网格不一致。
...
F_FNO 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：用分解式 Fourier 卷积沿各空间轴建模，相比标准 FNO 更强调各维度频域因子的高效组合。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：规则网格多维场预测，希望在 FNO 基础上控制谱卷积计算量的对比实验。。
- 当用户要求复用 `onescience.models.cfd_benchmark.F_FNO` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
- 不建议使用场景：点集无法合理投影到规则潜在网格，或 `shapelist` 与真实网格不一致。。
...
- 模型超参：源码读取字段 `act`, `fun_dim`, `geotype`, `modes`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/F_FNO.py`
