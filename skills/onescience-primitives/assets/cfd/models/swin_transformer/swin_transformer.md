...
- 任务类型：`cfd / model`
- 当前状态：`public`
- 主实现文件：`{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Swin_Transformer.py`

## 模型架构概览

Swin_Transformer 的架构定位是 `窗口注意力 Transformer`。它在 CFD_Benchmark 中作为可被 `model_factory.get_model(args, device)` 或直接模块导入使用的模型原语，主要服务于 CFD 场预测、网格/点级回归和多模型 benchmark 对比。组件基础定位可以概括为：仅支持 `structured_2D`，将网格 token padding 到窗口大小倍数后通过 SwinTransformerBlock 做窗口化注意力建模。

## 参数规模

参数规模不在源码中写死，主要由以下 `args` 字段和构造默认值决定：`act`, `fun_dim`, `geotype`, `n_heads`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- 主干结构：OneMlp 预处理 + `n_layers` 个 BasicLayer(SwinTransformerBlock depth=2) + Linear 输出头。
- 核心业务入口：`structured_geo`, `forward`。
- 源码类：`Model`；若存在辅助类，则包括 `BasicLayer, Model`。
...
主干计算
  点级 token 重排为 `(Batch, n_hidden, H, W)`，按 window_size=4 padding，Swin 后裁剪回原分辨率并展平输出。
...
from onescience.models.cfd_benchmark import Swin_Transformer
import torch
...
model = Swin_Transformer.Model(args, device).to(device)
...
- 支持范围：规则二维网格上比较窗口局部注意力与全局注意力/谱算子的任务。
- 限制条件：非结构化、1D 或 3D 几何；源码会对非 `structured_2D` 抛出异常。
...
Swin_Transformer 的规划决策知识用于指导 agent 判断何时选择该模型、如何生成配置、如何准备数据接口，以及失败时如何切换到辅助模型或适配器。它的组件定位是：仅支持 `structured_2D`，将网格 token padding 到窗口大小倍数后通过 SwinTransformerBlock 做窗口化注意力建模。 在多模型 CFD_Benchmark 任务中，应以该主模型的输入协议和源码读取的 args 字段为核心，其它模型仅作为性能、结构或适配策略参考。

### 适用场景

- 当任务满足：规则二维网格上比较窗口局部注意力与全局注意力/谱算子的任务。。
- 当用户要求复用 `onescience.models.cfd_benchmark.Swin_Transformer` 或进行 CFD_Benchmark 多模型对比。
- 当 datapipe 能提供源码要求的关键字段：`act`, `fun_dim`, `geotype`, `n_heads`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
- 不建议使用场景：非结构化、1D 或 3D 几何；源码会对非 `structured_2D` 抛出异常。。
...
- 模型超参：源码读取字段 `act`, `fun_dim`, `geotype`, `n_heads`, `n_hidden`, `n_layers`, `out_dim`, `ref`, `shapelist`, `space_dim`, `time_input`, `unified_pos`。
...
- `{onescience_path}/onescience/src/onescience/models/cfd_benchmark/Swin_Transformer.py`
