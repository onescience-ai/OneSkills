# 高保真汽车数据集全场与气动力联合代理（CFD_S011）

## 任务描述

面向WindsorML与DrivAerStar与CarBench汽车CFD数据完成高保真汽车数据集全场与气动力联合代理。产出可复现模型、任务结果、物理一致性评估和适用域报告；域外工况需经CFD复核。

## 工作流

### 步骤 1：数据接入与契约核验

**目的**：接入WindsorML与DrivAerStar与CarBench汽车CFD数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `WindsorML与DrivAerStar与CarBench汽车CFD数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“高保真汽车数据集全场与气动力联合代理”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Neural CFD surrogate完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Neural CFD surrogate` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Neural CFD surrogate，和{TRAIN_CONFIG}训练“高保真汽车数据集全场与气动力联合代理”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **WindsorML_ High-Fidelity Computational Fluid Dynamics Dataset For Automotive Aerodynamics** — sha256:f0f590a3d63ba4e4357ab61f45efdc5d29b5f2f2d8bb4bf1243fa3d902c9151e
2. **CarBench_ A Comprehensive Benchmark for Neural Surrogates on High-Fidelity 3D Car Aerodynamics** — [sha256:653bf51e34585036ffee19733c52ec4b5f5e356af5f0698ed1fef2fb46b1cad5](https://arxiv.org/abs/2512.07847)
3. **DrivAerStar_ An Industrial-Grade CFD Dataset for Vehicle Aerodynamic Optimization** — [sha256:5815a35dd5f2f019cc2f93550df4ebee2b098b4e24153c267aaae43d1c0b978b](https://arxiv.org/abs/2510.16857)
4. **Extreme Aerodynamics _ A Data-Driven Perspective** — [sha256:5edafc9413cf0f80f372df6075ca826b3a089fbd513ae1e20359b195d84f8678](https://arxiv.org/abs/2511.12889)
