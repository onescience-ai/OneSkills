...
**目的**：接入复杂几何点云与表面采样数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `复杂几何点云与表面采样数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“点云与无网格网络复杂几何流场预测”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Point-cloud network、Mesh-free neural surrogate完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Point-cloud network、Mesh-free neural surrogate` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Point-cloud network、Mesh-free neural surrogate，和{TRAIN_CONFIG}训练“点云与无网格网络复杂几何流场预测”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **A point-cloud deep learning framework for prediction of fluid flow fields on irregular geometries** — [sha256:f6676095ac9760d864335a22144bee49297fdda71fac26e7bbc1a037be4d3c81](https://arxiv.org/abs/2010.09469)
2. **MAgNet_ Mesh Agnostic Neural PDE Solver** — sha256:bfae397eacbda8a37397d08b5e72b63c8a36e98b9889609cd068eeb61c95a803
