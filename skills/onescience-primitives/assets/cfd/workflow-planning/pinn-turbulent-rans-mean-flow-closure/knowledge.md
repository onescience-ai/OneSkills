...
**目的**：接入RANS方程与稀疏均值流湍流数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `RANS方程与稀疏均值流湍流数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“PINN湍流RANS均值流求解与闭合”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Turbulence-augmented PINN完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Turbulence-augmented PINN` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Turbulence-augmented PINN，和{TRAIN_CONFIG}训练“PINN湍流RANS均值流求解与闭合”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Generalizable turbulence closures across bluff-body shapes by PINN-based solver-agnostic training** — [sha256:234ce8c704e8ae6595eecf88c4e644c7872488f5cea63aebe53a2d73f639b42c](https://arxiv.org/abs/2607.04491)
2. **Physics-informed neural networks for solving Reynolds-averaged Navier–Stokes equations** — [sha256:bea89597b17bba001f66ea49d70aa1e486ec1f1427d9907229c1c318ac9b1da8](https://arxiv.org/abs/2107.10711)
3. **Physics-informed data based neural networks for two-dimensional turbulence** — [sha256:8a6f8e39be54e8780339c06e4ca4628d9866497db892447c59ac1a126ef0f4c4](https://arxiv.org/abs/2203.02555)
4. **Evolution of inertial particle clustering in decaying turbulence** — [sha256:dee9b7e113ef2cd66962106013d25c60e51276aaaf28fa715a3b4c3a804e050a](https://arxiv.org/abs/2608.30545)
