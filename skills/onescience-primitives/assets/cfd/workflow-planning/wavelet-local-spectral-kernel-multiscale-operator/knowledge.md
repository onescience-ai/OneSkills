...
**目的**：接入多尺度高频PDE场数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `多尺度高频PDE场数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“小波与局部谱核多尺度算子学习”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Wavelet Neural Operator、Localized-kernel operator完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Wavelet Neural Operator、Localized-kernel operator` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Wavelet Neural Operator、Localized-kernel operator，和{TRAIN_CONFIG}训练“小波与局部谱核多尺度算子学习”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Multiwavelet-based Operator Learning for Differential Equations** — [sha256:ef1319257924132664542e00c4b842c4b9b11ccb8226769beb9cf450d63f4707](https://arxiv.org/abs/2109.13459)
2. **DRIFT-Net_ A Spectral-Coupled Neural Operator for PDEs Learning** — sha256:868fbd6e0db2336881254b7eb1c6d950a0fca4f1ceaddb2912cb552d596e3a0e
3. **Neural Operators with Localized Integral and Differential Kernels** — sha256:fe5f2d0c58e8be0411a5f5bbd54097956b2236caf051872e3558084e327de1db
4. **Gaussian Plane-Wave Neural Operator for Electron Density Estimation** — sha256:2717fba8cbbd32026fa9e04d00bd3aee67e8e5dacb41329c5f439383108a299f
5. **Learning High-Frequency Functions Made Easy with Sinusoidal Positional Encoding** — sha256:aacbb801ea6b287e78613d4f9b6d22c6be2f3b10e483bd760f235aee21c6ef93
6. **Spectral-Embedded Operator Learning for Three-Phase Interfacial Flow_ A Ternary Cahn-Hilliard-Navier-Stokes Benchmark** — [sha256:23e2d8826fd61913b32ba2ddd559de31ae5089b7bd807cc2476494f498952753](https://arxiv.org/abs/2608.29069)
