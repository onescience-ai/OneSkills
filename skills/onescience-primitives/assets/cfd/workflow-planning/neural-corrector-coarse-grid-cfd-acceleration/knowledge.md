...
**目的**：接入粗细网格CFD成对轨迹，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `粗细网格CFD成对轨迹` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“神经校正器耦合粗网格CFD加速”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Neural corrector、Hybrid PDE solver完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Neural corrector、Hybrid PDE solver` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Neural corrector、Hybrid PDE solver，和{TRAIN_CONFIG}训练“神经校正器耦合粗网格CFD加速”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **INC_ An Indirect Neural Corrector for Auto-Regressive Hybrid PDE Solvers** — [sha256:238e39181318fcb8b0b8dbeaac2c4e9cbc1114fc2c2b0f18f0d2067101fb6a80](https://arxiv.org/abs/2511.12764)
2. **MultiPDENet_ PDE-embedded Learning with Multi-time-stepping for Accelerated Flow Simulation** — sha256:901b7822a002740338eda713955c8a46c3f352043a573ef5524cffc93da61515
3. **P2C2Net_ PDE-Preserved Coarse Correction Network for efficient prediction of spatiotemporal dynamics** — sha256:da4451a2580c4666a798327c998ee68b52d178f1cb9165a6df998a2aba575527
4. **Toward Efficient Kernel-Based Solvers for Nonlinear PDEs** — sha256:a66ce5ca13bf9fd99c602d757e9dbb1b9dd368c2c4bde966ecda81ce9348e319
5. **Accelerating Legacy Numerical Solvers by Non-intrusive Gradient-based Meta-solving** — sha256:f52afc640a693abbec139f671e9ad096d9e80f2e25076cde637f34c8da722355
6. **Neural Network Approximations of PDEs Beyond Linearity_ A Representational Perspective** — sha256:8b22c58612c1b9ba804d37d884ac0af453ec3fa1c86278a44d499ae9d1a01cae
