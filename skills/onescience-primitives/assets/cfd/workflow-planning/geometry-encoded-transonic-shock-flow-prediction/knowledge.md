# 几何编码网络跨声速翼型激波流场预测（CFD_S004）

## 任务描述

面向跨声速超临界翼型CFD数据完成几何编码网络跨声速翼型激波流场预测。产出可复现模型、任务结果、物理一致性评估和适用域报告；域外工况需经CFD复核。

## 工作流

### 步骤 1：数据接入与契约核验

**目的**：接入跨声速超临界翼型CFD数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `跨声速超临界翼型CFD数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“几何编码网络跨声速翼型激波流场预测”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Geometry-encoded neural surrogate完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Geometry-encoded neural surrogate` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Geometry-encoded neural surrogate，和{TRAIN_CONFIG}训练“几何编码网络跨声速翼型激波流场预测”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Prediction of transonic flow over supercritical airfoils using geometric encoding and deep-learning strategies** — sha256:87ad00b604500fe228b6ab4a9d8e4e07be8c88a4fcd403f13023035c5011b0c4
