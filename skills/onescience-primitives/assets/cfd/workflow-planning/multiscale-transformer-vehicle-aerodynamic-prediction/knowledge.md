# 多尺度Transformer汽车高分辨率气动预测（CFD_S012）

## 任务描述

面向三维汽车表面与体流场数据完成多尺度Transformer汽车高分辨率气动预测。产出可复现模型、任务结果、物理一致性评估和适用域报告；域外工况需经CFD复核。

## 工作流

### 步骤 1：数据接入与契约核验

**目的**：接入三维汽车表面与体流场数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `三维汽车表面与体流场数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“多尺度Transformer汽车高分辨率气动预测”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练DragSolver、Transformer surrogate、Triplane network完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `DragSolver、Transformer surrogate、Triplane network` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认DragSolver、Transformer surrogate、Triplane network，和{TRAIN_CONFIG}训练“多尺度Transformer汽车高分辨率气动预测”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **DragSolver_ A Multi-Scale Transformer for Real-World Automotive Drag Coefficient Estimation** — sha256:e350073d6bb74bc6a116caa1e7a27bc85f7835f696f142e783f71c993a96f5f1
2. **PMRT_ A Training Recipe for Fast, 3D High-Resolution Aerodynamic Prediction** — [sha256:fad9528a67648d521f7d2316c88b97b3a1b9d24982916c88acda8b0ec39b7452](https://arxiv.org/abs/2509.17182)
3. **A Geometry-Aware Triplane Field Network for Vehicle Aerodynamic Prediction** — [sha256:9c9ff9867e1a3437b9ece3db9cb89e66f10328239371a0409e389c6583359020](https://arxiv.org/abs/2606.07724)
