...
**目的**：接入翼型壁面压力剪切及速度测量数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `翼型壁面压力剪切及速度测量数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“壁面压力剪切与积分气动力联合反演”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `CNN、Aerodynamic foundation model` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认CNN、Aerodynamic foundation model，和{TRAIN_CONFIG}训练“壁面压力剪切与积分气动力联合反演”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Predicting the wall-shear stress and wall pressure through convolutional neural networks** — [sha256:eb3fccb4f2c8b2eb8fb0e467a9d6ecce7ca2f40d75a4f0610150f2a6eeee535c](https://arxiv.org/abs/2303.00706)
2. **Data-driven estimation of scalar quantities from planar velocity measurements by deep learning applied to temper** — [sha256:d77ec0731eec33aaa2583ce763c951e931599723fa86f0c58e280a903ccce7b8](https://doi.org/10.21203/rs.3.rs-3306092/v1)
3. **Towards a Foundation-Model Paradigm for Aerodynamic Prediction in Three-dimensional Design** — [sha256:399fed5e0f6334454e9b0367446ef1182782f30697bd1a228030d436f08f8766](https://arxiv.org/abs/2604.18062)
