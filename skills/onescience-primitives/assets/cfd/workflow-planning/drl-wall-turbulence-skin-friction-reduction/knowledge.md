...
**目的**：接入槽道DNS壁面观测与吹吸控制数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `槽道DNS壁面观测与吹吸控制数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“深度强化学习壁湍流摩阻削减”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Deep reinforcement learning、Explainable CNN完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Deep reinforcement learning、Explainable CNN` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Deep reinforcement learning、Explainable CNN，和{TRAIN_CONFIG}训练“深度强化学习壁湍流摩阻削减”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Deep reinforcement learning for turbulent drag reduction in channel flows** — [sha256:621ea8446784760a86ef23dcad7ab523a9aee32f8511735d5bf0e0445eb9c808](https://arxiv.org/abs/2301.09889)
2. **Identifying regions of importance in wall-bounded turbulence through explainable deep learning** — [sha256:76cd6545dbe190fcf6663771acca0d3c40fc1c3f10fd4df6c970c38ac92495a8](https://arxiv.org/abs/2302.01250)
