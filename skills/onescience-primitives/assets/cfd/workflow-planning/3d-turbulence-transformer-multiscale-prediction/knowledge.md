# 三维湍流Transformer多尺度时空预测（CFD_S024）
...
**目的**：接入三维DNS与LES体湍流数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `三维DNS与LES体湍流数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“三维湍流Transformer多尺度时空预测”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练EddyFormer、Mesh Transformer完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `EddyFormer、Mesh Transformer` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认EddyFormer、Mesh Transformer，和{TRAIN_CONFIG}训练“三维湍流Transformer多尺度时空预测”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **EAGLE_ Large-scale Learning of Turbulent Fluid Dynamics with Mesh Transformers** — [sha256:96cf2c58d5de9a24df6144504b92738136c9ef37bdc34f1debc286bc72e01929](https://arxiv.org/abs/2302.10803)
2. **EddyFormer_ Accelerated Neural Simulations of Three-Dimensional Turbulence at Scale** — [sha256:37b4f81cbb2343c333baa13e95794398a2a751f00a1a3951d9213c6973e099de](https://arxiv.org/abs/2510.24173)
3. **Erwin_ A Tree-based Hierarchical Transformer for Large-scale Physical Systems** — sha256:2d7089291384b6e06839581066852840882be88e8adbbf9099be7c58fc05231e
