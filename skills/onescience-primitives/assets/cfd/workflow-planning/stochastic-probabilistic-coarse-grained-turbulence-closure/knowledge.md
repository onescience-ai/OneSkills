...
**目的**：接入多尺度湍流粗细状态轨迹，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `多尺度湍流粗细状态轨迹` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“随机微分与概率粗粒化湍流闭合”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Neural SDE closure、Probabilistic coarse-graining model完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Neural SDE closure、Probabilistic coarse-graining model` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Neural SDE closure、Probabilistic coarse-graining model，和{TRAIN_CONFIG}训练“随机微分与概率粗粒化湍流闭合”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **A physics-aware, probabilistic machine learning framework for coarse-graining high-dimensional systems in the Sm** — [sha256:c0c097db576f873bee6449ca4bf785cb7aedfd354a80d3b8c346d52feeedb079](https://arxiv.org/abs/1902.03968)
2. **Neural Ideal Large Eddy Simulation_ Modeling Turbulence with Neural Stochastic Differential Equations** — sha256:9f63401bd844481bbad5309f43bd3797527e3549028fb2469b442499bbf6a89d
3. **Learning Stochastic Multiscale Models** — [sha256:f164b0f083c5efdc9bceb70cef28369d6b74a1c25b432487c88fefdb4f4a962d](https://arxiv.org/abs/2506.22655)
4. **Echo state network for two-dimensional turbulent moist Rayleigh-Bénard convection** — [sha256:faf3c9138e2ab046be096fbe5508f9c806eed3be969fd87198bb20568b57e601](https://arxiv.org/abs/2101.11325)
5. **Machine-learning energy-preserving nonlocal closures for turbulent fluid flows and inertial tracers** — [sha256:40324ee9bc4804d38ec8af85e77d587281aa87143cd544ef25b9031a16d52044](https://doi.org/10.1103/PhysRevFluids.7.024305)
