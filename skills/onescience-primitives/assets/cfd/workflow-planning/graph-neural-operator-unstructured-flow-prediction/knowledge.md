# 图神经算子任意域非结构网格流场预测（CFD_S007）

## 任务描述

面向任意几何非结构网格CFD数据完成图神经算子任意域非结构网格流场预测。产出可复现模型、任务结果、物理一致性评估和适用域报告；域外工况需经CFD复核。

## 工作流

### 步骤 1：数据接入与契约核验

**目的**：接入任意几何非结构网格CFD数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `任意几何非结构网格CFD数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“图神经算子任意域非结构网格流场预测”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Graph Neural Operator、MeshGraphNet完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Graph Neural Operator、MeshGraphNet` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Graph Neural Operator、MeshGraphNet，和{TRAIN_CONFIG}训练“图神经算子任意域非结构网格流场预测”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **RIGNO_ A Graph-based Framework For Robust And Accurate Operator Learning For PDEs On Arbitrary Domains** — [sha256:2cc63bcf4945d9f1a546e8f3cf00c85c5e6bfc69b6fc079c4a6266a92f03c214](https://arxiv.org/abs/2501.19205)
2. **PIORF_ Physics-Informed Ollivier-Ricci Flow for Long–Range Interactions in Mesh Graph Neural Networks** — sha256:248a0ce6c1fceeb494ee6d44ca85f9cde0ca3c6092c21fcca5e2e6545a5eafcf
3. **Finite Volume Features, Global Geometry Representations, and Residual Training for Deep Learning-based CFD Simul** — sha256:4cbb78a5988ddb3f7a73358a765c5878b5e0ff58c603b37cccc8057a7962ddbe
4. **Efficient Learning of Mesh-Based Physical Simulation with Bi-Stride Multi-Scale Graph Neural Network** — sha256:17368e363f02dcb1945361e79608ed7868e16fed7963d17b69f286472159a219
