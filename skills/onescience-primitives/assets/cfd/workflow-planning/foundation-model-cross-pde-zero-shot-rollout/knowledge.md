...
**目的**：接入多物理预训练PDE时序数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `多物理预训练PDE时序数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“基础模型跨PDE零样本时序预测”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练PDE foundation model、In-context learner完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `PDE foundation model、In-context learner` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认PDE foundation model、In-context learner，和{TRAIN_CONFIG}训练“基础模型跨PDE零样本时序预测”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Zebra_ In-Context Generative Pretraining for Solving Parametric PDEs** — sha256:23aac715c15a412fb59016b5c283a3233a9f3f131ce5bd48f7d224cb3658fb18
2. **Physics-informed Temporal Alignment for Auto-regressive PDE Foundation Models** — sha256:f2696bf4c79c52a014e7f9051224670c128f7dc024d909006781c938df23405c
3. **Zero-shot forecasting of chaotic systems** — sha256:f9d0475069d7ca038747471ccab0e7f352caee2fc5abed442766143adf2eaa45
4. **Multiple Physics Pretraining for Spatiotemporal Surrogate Models** — sha256:47661de04057b8bfd18e716afe3af062131bae139b45571881ee6cad3a033623
5. **MetaPhysiCa_ Improving OOD Robustness in Physics-informed Machine Learning** — sha256:9fd731eab12eaf7c991148d9d969bd196c160f51079d9a23bb92ba21f23fecce
6. **FLUID-LLM_ Learning Computational Fluid Dynamics with Spatiotemporal-aware Large Language Models** — [sha256:59ce4fdea6d50af118ef9d30e46677d8080f03e037f6863b511f6895d0ba6b93](https://arxiv.org/abs/2406.04501)
