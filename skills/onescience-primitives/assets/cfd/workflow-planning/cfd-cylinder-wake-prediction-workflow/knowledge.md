...
## 适用范围
圆柱绕流非定常时序数据的自回归网络长时滚动预测，覆盖从数据接入到验收的完整工作流。适用于采用 Autoregressive CNN 或 PDE-Refiner 等神经网络模型对圆柱尾迹进行多步滚动预测的任务。
...
- 模型名称（{MODEL_NAME}，默认 Autoregressive CNN、PDE-Refiner）
- 训练配置（{TRAIN_CONFIG}）
...
## 资源召回建议
- 本卡片为工作流级卡片，可被以下需求召回：圆柱绕流预测流程、自回归CFD工作流、CFD数据驱动预测流程
