...
**目的**：接入Rayleigh-Benard对流与标量混合数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `Rayleigh-Benard对流与标量混合数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“对流与标量混合湍流热通量预测”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Reservoir computer、CNN完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Reservoir computer、CNN` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Reservoir computer、CNN，和{TRAIN_CONFIG}训练“对流与标量混合湍流热通量预测”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Direct data-driven forecast of local turbulent heat flux in Rayleigh–Bénard convection** — [sha256:40922c1e48b0aba37d3f08c8ef896e9010715af0d6f6c0b22152319c85a03a55](https://arxiv.org/abs/2202.13129)
2. **Reservoir computing model of two-dimensional turbulent convection** — [sha256:3f648f18ff486ba5221655ef68783e6ef63c2c7bdc77986c70b7f94ff0f8cd57](https://arxiv.org/abs/2001.10280)
3. **Deep learning of turbulent scalar mixing** — [sha256:2070578e7dd3dd8ced4582e8c4bc9c0d61ae2696046dacd0e5318f895f66c6b0](https://arxiv.org/abs/1811.07095)
