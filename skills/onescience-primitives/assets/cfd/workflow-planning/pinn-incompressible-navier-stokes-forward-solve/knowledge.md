# PINN不可压Navier-Stokes正问题求解（CFD_S036）
...
**目的**：接入不可压层流Navier-Stokes配置与配点，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `不可压层流Navier-Stokes配置与配点` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“PINN不可压Navier-Stokes正问题求解”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练PINN、NSFnet完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `PINN、NSFnet` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认PINN、NSFnet，和{TRAIN_CONFIG}训练“PINN不可压Navier-Stokes正问题求解”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **NSFnets (Navier–Stokes Flow Nets): Physics-Informed Neural Networks for the Incompressible Navier–Stokes Equations** — [sha256:dda79e70937a72a49fdb4ed78ff901dc1fb74bc9c3e578998e120ff57e94710a](https://arxiv.org/abs/2003.06496)
2. **Physics-informed deep learning for incompressible laminar flows** — [sha256:90e03aa845958478207b0287ab6082f77c1b5332aa972f24e3eb104cca15ca26](https://arxiv.org/abs/2002.10558)
3. **Self-adaptive loss balanced Physics-informed neural networks for the incompressible Navier-Stokes equations** — [sha256:317a29dec1b14c712d4639d3a9defa3038f55f4e1387d7f4992b2bb7703ec1ca](https://doi.org/10.1007/s10409-021-01053-7)
4. **Complementary, Not Cumulative_ Interaction Effects in Physics-Informed Neural Networks for Navier - Stokes Vortex Shedding** — [sha256:077527360c3cb70c7da03cb5935dd065ba335a69e1cf6df9d1d1ce90da17cb3d](https://arxiv.org/abs/2608.19632)
5. **NeuralFlowNet_ Towards Data-Free Physics-Informed Neural Network Solutions of Navier-Stokes Equations Across Low and High Reynolds Numbers** — [sha256:a8de78631291eeb3163da2096f0b2368716dbfb268610c1d6cc49954b07ca410](https://arxiv.org/abs/2608.28935)
