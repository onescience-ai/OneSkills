# CycleGAN无配对LES到DNS超分辨率重建（CFD_S099）
...
**目的**：接入非配对LES与DNS湍流数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `非配对LES与DNS湍流数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“CycleGAN无配对LES到DNS超分辨率重建”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练CycleGAN完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `CycleGAN` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认CycleGAN，和{TRAIN_CONFIG}训练“CycleGAN无配对LES到DNS超分辨率重建”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Unsupervised deep learning for super-resolution reconstruction of turbulence** — [sha256:3c8cd311e4cdd6112148d0c220e9e5196e49f35eb4af73cfb7d3a5d534467a43](https://arxiv.org/abs/2007.15324)
2. **A neural network approach for the blind deconvolution of turbulent flows** — [sha256:92ee9bc6830b8ee1835671b6807e937dcf82966689c8a0d699989dd2597d1847](https://arxiv.org/abs/1706.00912)
