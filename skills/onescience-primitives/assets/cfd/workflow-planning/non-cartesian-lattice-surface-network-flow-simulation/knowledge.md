...
**目的**：接入非笛卡尔晶格与几何曲面流动数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `非笛卡尔晶格与几何曲面流动数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“非笛卡尔晶格与曲面网络流动模拟”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Non-Cartesian CNN、Surface neural simulator完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Non-Cartesian CNN、Surface neural simulator` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Non-Cartesian CNN、Surface neural simulator，和{TRAIN_CONFIG}训练“非笛卡尔晶格与曲面网络流动模拟”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **NCDL_ A Framework for Deep Learning on non-Cartesian Lattices** — sha256:40d4dad6d272b8e1c2818e2104312a9a3f57c7697479d98ed12b3c62acd5ea5f
2. **Geometry Aware Operator Transformer as an efficient and accurate neural surrogate for PDEs on arbitrary domains** — [sha256:f8c8526129008c7badd1b20f8ed2ffc82864b0c43aba34bac8079e79474188d8](https://arxiv.org/abs/2505.18781)
3. **Neural Fluid Simulation on Geometric Surfaces** — sha256:c4d52db61ee43b7c729a4b63d83bf1f8f784de9101f88aa56a682a78ac9eeffe
4. **Conditionally Parameterized, Discretization-Aware Neural Networks for Mesh-Based Modeling of Physical System** — [sha256:fdaaca1b8de998741453dc55f91df63acb1fab54dee10c04f1eebc9f46c9cd16](https://arxiv.org/abs/1812.08373)
