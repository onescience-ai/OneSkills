...
**目的**：接入涡激振动与流固耦合轨迹数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `涡激振动与流固耦合轨迹数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“流固耦合移动网格振动时序预测”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练ALE neural network、Graph simulator完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `ALE neural network、Graph simulator` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认ALE neural network、Graph simulator，和{TRAIN_CONFIG}训练“流固耦合移动网格振动时序预测”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Neural Latent Arbitrary Lagrangian-Eulerian Grids for Fluid-Solid Interaction** — sha256:934c0ac430579c3717e3891429c9f592a8c615b3fe8e33a6217db9ab9b867348
2. **Deep learning of vortex-induced vibrations** — [sha256:2f9cfd53fe4d6cf4c6f268131e8ba56ff2c46055f24b3cce9021aa62f882d955](https://arxiv.org/abs/1808.08952)
