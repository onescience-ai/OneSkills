...
**目的**：接入亚声速至超声速可压缩绕流数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `亚声速至超声速可压缩绕流数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“物理网络可压缩高速绕流求解与激波识别”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Physics-informed neural network、Shock classifier完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Physics-informed neural network、Shock classifier` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Physics-informed neural network、Shock classifier，和{TRAIN_CONFIG}训练“物理网络可压缩高速绕流求解与激波识别”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Solving nonlinear subsonic compressible flow in infinite domain via multi-stage neural networks** — [sha256:c95e55d5c33138ef0aa29c5d57830035579f08c82fb8c1a7bccad3e0f8c131f4](https://arxiv.org/abs/2601.00342)
2. **An unsupervised machine-learning-based shock sensor for high-order supersonic flow solvers** — [sha256:0e4d4bd489b6bdec29c3d4a487b5d7e775f6289dbe6f3e337b38e72a2996f0c0](https://arxiv.org/abs/2308.00086)
3. **Supervised machine learning of compressible flow past a rotating cylinder** — [sha256:15b6be3df8b3d7b238933265eef26079d4c8cd48afdbdfc628f5023d1d7b4adc](https://arxiv.org/abs/2605.26806)
