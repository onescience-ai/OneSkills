...
**目的**：接入显式约束动力系统时序数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `显式约束动力系统时序数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“稳定性约束神经微分方程动力学学习”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Stable Neural ODE、Constrained neural process完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Stable Neural ODE、Constrained neural process` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Stable Neural ODE、Constrained neural process，和{TRAIN_CONFIG}训练“稳定性约束神经微分方程动力学学习”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Neural Processes with Stability** — sha256:173d3a6fa7190254cececc45fffd10fe965a4bdc022afa92d2f031d18014efcb
2. **Axial Neural Networks for Dimension-Free Foundation Models** — [sha256:ba35ae1521fa9b6aa91ba3c509cab2f82307e8bd9ecacc7890d8a2a3cd974df9](https://arxiv.org/abs/2510.13665)
3. **HHD-GP_ Incorporating Helmholtz-Hodge Decomposition into Gaussian Processes for Learning Dynamical Systems** — sha256:a0fde0f1083442395a40db558b83ef573d1d78b8da4b443f744e8097fae3353e
4. **Stabilized Neural Differential Equations for Learning Dynamics with Explicit Constraints** — sha256:3f8029229878482b5c0a451a2675a373d05f3c99347a3fe4711b277dafc1c76a
5. **Spatio-Temporal Prediction of Unsteady Airfoil Aerodynamics Using Augmented Graph Neural Ordinary Differential Equations with Exogenous Controls** — [sha256:d74179a147d74c9af218e6ada73b61e5ae098f3dcb01b3d9cf7583de121129fb](https://arxiv.org/abs/2607.18309)
