...
## 适用范围
本卡片覆盖汽车外流场空气动力学中，基于多个高保真汽车CFD数据集（WindsorML、DrivAerStar、CarBench）构建全场物理量与气动力系数联合代理模型的完整任务场景。适用于需要同时预测全场流场分布（压力场、速度场、湍动能场等）与全局气动力系数（阻力系数Cd、升力系数Cl等）的汽车空气动力学深度学习任务。不适用于内部流动、热管理或瞬态涡脱落分析等场景。

## 输入
- **几何数据**：汽车三维表面网格或点云，含坐标与边界信息
- **工况数据**：来流速度、雷诺数、攻角等（视数据集而定）
- **目标变量**：全场物理量（压力、速度分量、湍动能等）+ 气动力系数（Cd、Cl等）
- **数据集**：WindsorML、DrivAerStar、CarBench中的一个或多个
- **数据许可**：需遵循各数据集使用条款

## 输出
- 训练好的联合代理模型（best_checkpoint.pt）
- 测试集逐样本全场预测与气动力系数预测
...
**s01 数据接入与契约核验**：
- 操作：接入WindsorML/DrivAerStar/CarBench数据，核验样本数、变量、单位、网格坐标及许可
