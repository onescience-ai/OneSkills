...
**目的**：接入多孔介质微流动与输运CFD数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `多孔介质微流动与输运CFD数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“多孔与复杂输运流动场数据驱动预测”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练CNN surrogate、Neural differential equation完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `CNN surrogate、Neural differential equation` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认CNN surrogate、Neural differential equation，和{TRAIN_CONFIG}训练“多孔与复杂输运流动场数据驱动预测”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Data-Driven Design Optimization of Streaming-Potential-Mediated Electrokinetic Transport of Viscoelastic Fluids in Microchannels** — [sha256:8116a396502a535856641a3f3a732c7f3d326a6e4df47ec34b01f4f0df2e0f1b](https://arxiv.org/abs/2608.29939)
2. **Advances in Scientific Machine Learning for Coupled Fluid Flow and Transport** — [sha256:a51cb26e72c0434865adf4c649d1d8022b327093e8a02e154dc875aa86b69727](https://arxiv.org/abs/2606.19562)
3. **Online Gate-Driven Flow Control in Resin Transfer Moulding Using a Neural -Network Surrogate** — [sha256:676b35cd9b7e95297ffe45a90aaa9841573b3660bc7b47007ad6dda6c479435f](https://arxiv.org/abs/2608.29521)
4. **Turbulent Microscale Flow Field Prediction In Porous Media Using Convolutional Neural Networks** — sha256:c6db3a95540f2fbcf08fb764810e428375010f758649ba9efadbecfb6cfdc95c
