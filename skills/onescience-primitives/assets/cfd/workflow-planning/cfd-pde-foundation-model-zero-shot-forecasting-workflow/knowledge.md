...
## 适用范围
适用于利用预训练PDE基础模型对未知PDE工况进行零样本时序预测的标准工作流。涵盖从数据接入到验收评估的完整流程，强调物理一致性和适用域判定。不适用于需要大量下游微调的场景，或物理约束不明确的纯数据驱动预测。
...
## 资源召回建议
当用户需要进行跨PDE零样本时序预测工作流时召回本卡片。配套资源包括：PDE数据集、基础模型实现、训练框架、评估工具。相关卡片：cfd-pde-foundation-model-zero-shot-forecasting-scenario（场景级）、cfd-data-intake-contract-validation（数据接入任务）、cfd-preprocessing-data-splitting（预处理任务）、cfd-model-training（模型训练任务）、cfd-batch-inference-physical-recovery（推理任务）、cfd-acceptance-applicability（验收任务）。
