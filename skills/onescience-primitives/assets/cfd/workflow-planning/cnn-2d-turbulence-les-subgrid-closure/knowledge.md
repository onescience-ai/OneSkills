...
**目的**：接入二维湍流DNS滤波与LES数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `二维湍流DNS滤波与LES数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“卷积网络二维湍流LES亚格子闭合”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练CNN SGS model完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `CNN SGS model` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认CNN SGS model，和{TRAIN_CONFIG}训练“卷积网络二维湍流LES亚格子闭合”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **Data-driven subgrid-scale modeling of forced Burgers turbulence using deep learning with generalization to h** — [sha256:5bed8d70e690151c0eadb40916dfd8c79f97ebcbfeed16b5ca79ac4cadaa426b](https://arxiv.org/abs/2012.06664)
2. **Subgrid modelling for two-dimensional turbulence using neural networks** — [sha256:a8bfb15f49f7cd719c42e00b956113a800d9c0a2c834ecd66bdc7b4652908596](https://arxiv.org/abs/1808.02983)
3. **Stable a Posteriori LES of 2D Turbulence Using Convolutional Neural Networks: Backscattering Analysis and Generalization to Higher Re via Transfer Learning** — [sha256:61462961b6643c1388f1ed3929c11558c6cb028b9b9098a7fc759c457af196a3](https://arxiv.org/abs/2102.11400)
4. **Sub-grid scale model classification and blending through deep learning** — [sha256:77a607156c6662652b606da745ac1c13d2ebefacaaea0f28da92135f9744623d](https://arxiv.org/abs/1812.11949)
