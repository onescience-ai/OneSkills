# 多尺度Transformer汽车高分辨率气动预测工作流

## 适用范围
本卡片覆盖基于多尺度Transformer架构完成三维汽车表面与体流场高分辨率气动预测的标准化五步工作流，适用于从数据接入到任务验收的完整流程。工作流需同时保证全场预测精度与物理一致性，并评估泛化能力与工程可用性。

## 输入
- **数据集路径**：三维汽车表面与体流场数据集目录或清单文件
- **数据集名称**：来源与数据版本标识
- **数据契约**：变量、单位、网格定义（可选）
- **切分配置**：按对象/工况切分比例与种子
- **模型配置**：框架、超参数、随机种子
...
## 资源召回建议
当用户涉及以下任务时应召回本卡片：
- 多尺度Transformer汽车气动预测工作流
- DragSolver/Transformer surrogate/Triplane network模型训练与评估流程
- 汽车全场流场高分辨率预测工作流
