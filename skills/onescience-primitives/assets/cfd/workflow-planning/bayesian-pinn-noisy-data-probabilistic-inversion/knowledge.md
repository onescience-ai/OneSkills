# 贝叶斯PINN噪声数据概率反演（CFD_S039）

## 任务描述

面向含噪观测与PDE先验数据完成贝叶斯PINN噪声数据概率反演。产出可复现模型、任务结果、物理一致性评估和适用域报告；域外工况需经CFD复核。

## 工作流

### 步骤 1：数据接入与契约核验

**目的**：接入含噪观测与PDE先验数据，核验样本、变量、单位、网格坐标及许可。

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 数据集路径 | `{DATASET_PATH}` | doc | 是 | — | 目录或清单文件 |
| 数据集名称 | `{DATASET_NAME}` | str | 是 | `含噪观测与PDE先验数据` | 来源与数据版本 |
...
**操作指令**：`读取{DATASET_PATH}中的{DATASET_NAME}，为“贝叶斯PINN噪声数据概率反演”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
**目的**：训练Bayesian PINN完成指定输入到目标物理量的映射。

**依赖步骤**：s02

| 参数 | 变量名 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|---|
| 模型名称 | `{MODEL_NAME}` | str | 是 | `Bayesian PINN` | 实现或模型注册名 |
...
**操作指令**：`使用{MODEL_NAME}，默认Bayesian PINN，和{TRAIN_CONFIG}训练“贝叶斯PINN噪声数据概率反演”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。`
...
1. **B-PINNs_ Bayesian physics-informed neural networks for forward and inverse PDE problems with noisy data** — [sha256:71ed71a022f5ceca1a1e2371afb8f076d4d7379fc00b77fa583a1db2086d466d](https://arxiv.org/abs/2003.06097)
2. **Bayesian Physics Informed Neural Networks for real-world nonlinear dynamical systems** — [sha256:817c02edda1742622168a7cd289b6b29dffee21e047d65d8a8c31d87397e51dd](https://arxiv.org/abs/2205.08304)
3. **Adversarial uncertainty quantification in physics-informed neural networks** — [sha256:1b26978e9e3164703e632f8369291c7dcf7f09327fa088a24e4bf09df943b43b](https://arxiv.org/abs/1811.04026)
4. **Uncertainty-Aware Diagnostics for Physics-Informed Machine Learning** — sha256:2fff639fe754584d06c8ccceb910935a55acf66dc19682c1d8a121199efe0f97
