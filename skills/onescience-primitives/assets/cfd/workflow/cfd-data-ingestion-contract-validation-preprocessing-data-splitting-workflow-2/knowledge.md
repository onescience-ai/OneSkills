# 工作流：cfd-data-ingestion-contract-validation-preprocessing-data-splitting-workflow-2
...
### s01 数据接入与契约核验
- desc: 接入低高保真配对气动数据，核验样本、变量、单位、网格坐标及许可。
- depend: []
- prompt: 读取{DATASET_PATH}中的{DATASET_NAME}，为“贝叶斯多保真气动代理与主动选样”建立数据清单。检查文件可读性、样本数、输入与目标变量、单位、坐标系、网格拓扑、时间或工况范围、缺失值和使用许可，按{DATA_CONTRACT}输出机器可读契约。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。
- step_input:
...
### s03 模型配置与训练
- desc: 训练Bayesian surrogate、Gaussian process完成指定输入到目标物理量的映射。
- depend: ['s02']
- prompt: 使用{MODEL_NAME}，默认Bayesian surrogate、Gaussian process，和{TRAIN_CONFIG}训练“贝叶斯多保真气动代理与主动选样”模型。加载s02切分与统计量，记录代码版本、依赖、随机种子、逐轮训练验证指标与最佳权重。若提供{INIT_CHECKPOINT}须检查结构兼容性。缺少必填输入时返回BLOCKED并列出缺项，不得编造数据、权重、工况或结果。
- step_input:
...
### s04 概率推理与不确定性校准
- desc: 输出预测分布、置信区间和分布外不确定性并执行校准。
- depend: ['s03']
- prompt: 加载{CHECKPOINT}，对独立测试集及外推集执行概率推理。保存预测均值、方差或样本集合，区分认知与随机不确定性；不得用测试标签回调模型。报告覆盖率、区间宽度、NLL和校准误差。
- step_input:
...
- outputs: ['predictive_distribution/', 'uncertainty_decomposition.json', 'calibration.csv']
- quality_gate: ['概率输出有限且定义清楚', '测试集未参与校准训练', '区间覆盖率与宽度同时报告']
...
- step_input:
  - {METRICS} (required=True, type=list[str], var_name=验收指标, hint=统计和物理指标, default=['relative_L2', 'NLL', 'coverage', 'calibration_error'])
