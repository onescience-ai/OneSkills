...
本工作流覆盖从原始拉格朗日粒子轨迹数据到物理一致性验证的完整建模闭环。适用于：
- 基于E3等变图神经网络的流体动力学模拟
- 粒子表示的流体数据处理与建模
- 需要物理约束验证的深度学习流体模型
...
- {DATASET_NAME}: 数据集名称（必填，默认"拉格朗日粒子轨迹数据"）
...
**操作**：
1. 加载{MODEL_NAME}（默认E3-equivariant GNN + Lagrangian simulator）
...
[2] CFD_S062场景需求书, scenario_catalogs/fluid/, 2026