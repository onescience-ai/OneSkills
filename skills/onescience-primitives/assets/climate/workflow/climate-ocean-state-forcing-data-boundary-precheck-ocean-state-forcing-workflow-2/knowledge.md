# 工作流：climate-ocean-state-forcing-data-boundary-precheck-ocean-state-forcing-workflow-2
...
- desc: 界定“全球涡旋分辨率三维海洋1—10日预报”的任务范围并执行“海洋状态和强迫资料及任务边界预检”，核验三维海洋分析场、海表温度和海面风的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“全球涡旋分辨率三维海洋1—10日预报”，读取{OCEAN_STATE}、{SURFACE_FORCING}、{STATIC_OCEAN_DATA}、{START_TIME}、{TARGET_HORIZON}、{DATA_CUTOFF_TIME}并完成海洋状态和强迫资料及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
- desc: 执行“海洋状态和强迫资料时空对齐与样本构造”，统一三维海洋分析场、海表温度和海面风的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 生成各提前期三维海洋状态场
- desc: 按冻结配置执行“生成各提前期三维海洋状态场”，完成从三维海洋分析场、海表温度和海面风到多时效温盐流及海表高度场的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RUN_CONFIG}、{OUTPUT_INTERVAL}、{ENSEMBLE_SIZE}执行生成各提前期三维海洋状态场，将三维海洋分析场、海表温度和海面风转换为多时效温盐流及海表高度场。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['生成各提前期三维海洋状态场结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '输出在岸线、深水区和强梯度区无异常跳变']

### s05 发布预报场、质量标志和运行配置
- desc: 执行“发布预报场、质量标志和运行配置”，恢复物理量、坐标和元数据，生成多时效温盐流及海表高度场及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成发布预报场、质量标志和运行配置，对核心结果执行已登记的校准、订正、派生或聚合并输出多时效温盐流及海表高度场。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['多时效温盐流及海表高度场', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '输出在岸线、深水区和强梯度区无异常跳变']

### s06 检验原位误差、环流、涡旋、频谱和漂移
- desc: 执行“检验原位误差、环流、涡旋、频谱和漂移”，使用未参与参数选择的参考资料检验多时效温盐流及海表高度场，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验原位误差、环流、涡旋、频谱和漂移。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
