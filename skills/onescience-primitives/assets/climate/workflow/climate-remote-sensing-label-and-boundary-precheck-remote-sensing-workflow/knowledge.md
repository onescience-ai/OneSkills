# 工作流：climate-remote-sensing-label-and-boundary-precheck-remote-sensing-workflow
...
### s01 遥感观测标签及任务边界预检
- desc: 界定“30米大范围农田范围与面积制图”的任务范围并执行“遥感观测标签及任务边界预检”，核验多季节Landsat影像、植被指数和训练样本的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“30米大范围农田范围与面积制图”，读取{SENSOR_DATA}、{AUXILIARY_DATA}、{TARGET_REGION_TIME}、{REFERENCE_LABELS}、{QUALITY_MASK}并完成遥感观测标签及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
- quality_gate: ['所有必需输入均存在且路径、版本和来源可追溯', '任务区域、时段、变量和输出目标无歧义', '缺测、重复和异常资料已记录且未擅自补造', '传感器、轨道、处理级别和观测时间可追溯', '标签限定为农田二分类并明确最小制图单元']

### s02 遥感观测标签时空对齐与样本构造
- desc: 执行“遥感观测标签时空对齐与样本构造”，统一多季节Landsat影像、植被指数和训练样本的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 批量生成并拼接农田概率图
- desc: 按冻结配置执行“批量生成并拼接农田概率图”，完成从多季节Landsat影像、植被指数和训练样本到30米农田范围图及区域面积的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RUN_CONFIG}、{CROPLAND_THRESHOLD}、{AREA_STATISTICS_UNITS}执行批量生成并拼接农田概率图，将多季节Landsat影像、植被指数和训练样本转换为30米农田范围图及区域面积。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['批量生成并拼接农田概率图结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '输出坐标、分辨率、无效值和置信信息完整', '二分类概率、农田掩膜和面积统计同步生成']

### s05 输出农田概率、二值范围和面积统计
- desc: 执行“输出农田概率、二值范围和面积统计”，恢复物理量、坐标和元数据，生成30米农田范围图及区域面积及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出农田概率、二值范围和面积统计，对核心结果执行已登记的校准、订正、派生或聚合并输出30米农田范围图及区域面积。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['30米农田范围图及区域面积', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '输出坐标、分辨率、无效值和置信信息完整']

### s06 用混淆矩阵及统计资料检验空间和面积
- desc: 执行“用混淆矩阵及统计资料检验空间和面积”，使用未参与参数选择的参考资料检验30米农田范围图及区域面积，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成用混淆矩阵及统计资料检验空间和面积。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
...
- quality_gate: ['验证资料未参与训练、参数选择或阈值调优', '结果与基线采用相同样本、网格和指标协议', '未达到预先登记门限时不得判定性能通过', '配置、日志、产物路径和文件哈希可追溯', '验证采用独立地点、日期或传感器资料', '像元精度与行政区面积误差分别报告']
