# 工作流：climate-remote-sensing-label-and-boundary-precheck-remote-sensing-workflow-3
...
### s01 遥感观测标签及任务边界预检
- desc: 界定“Landsat水体叶绿素a概率反演”的任务范围并执行“遥感观测标签及任务边界预检”，核验Landsat水色波段、观测几何和水样叶绿素标签的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“Landsat水体叶绿素a概率反演”，读取{SENSOR_DATA}、{AUXILIARY_DATA}、{TARGET_REGION_TIME}、{REFERENCE_LABELS}、{QUALITY_MASK}并完成遥感观测标签及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
### s02 遥感观测标签时空对齐与样本构造
- desc: 执行“遥感观测标签时空对齐与样本构造”，统一Landsat水色波段、观测几何和水样叶绿素标签的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 反演叶绿素a条件分布或点估计
- desc: 按冻结配置执行“反演叶绿素a条件分布或点估计”，完成从Landsat水色波段、观测几何和水样叶绿素标签到叶绿素a浓度、分位数及质量掩膜的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RUN_CONFIG}、{DECISION_THRESHOLD}、{UNCERTAINTY_CONFIG}执行反演叶绿素a条件分布或点估计，将Landsat水色波段、观测几何和水样叶绿素标签转换为叶绿素a浓度、分位数及质量掩膜。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['反演叶绿素a条件分布或点估计结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '输出坐标、分辨率、无效值和置信信息完整']

### s05 输出浓度图、不确定性和适用域标志
- desc: 执行“输出浓度图、不确定性和适用域标志”，恢复物理量、坐标和元数据，生成叶绿素a浓度、分位数及质量掩膜及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出浓度图、不确定性和适用域标志，对核心结果执行已登记的校准、订正、派生或聚合并输出叶绿素a浓度、分位数及质量掩膜。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['叶绿素a浓度、分位数及质量掩膜', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '输出坐标、分辨率、无效值和置信信息完整']

### s06 按水体、浓度区间和留站样本验证
- desc: 执行“按水体、浓度区间和留站样本验证”，使用未参与参数选择的参考资料检验叶绿素a浓度、分位数及质量掩膜，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按水体、浓度区间和留站样本验证。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
