# 工作流：climate-near-observation-task-boundary-precheck-near-observation-space-workflow-5
...
### s01 临近观测及任务边界预检
- desc: 界定“机场雾和低能见度事件短时概率预报”的任务范围并执行“临近观测及任务边界预检”，核验机场温湿风、能见度历史、NWP或CAMS预报的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“机场雾和低能见度事件短时概率预报”，读取{RECENT_OBSERVATIONS}、{FORECAST_START_TIME}、{FORECAST_HORIZON}、{TARGET_VARIABLES}、{DATA_CUTOFF_TIME}并完成临近观测及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
### s02 临近观测时空对齐与样本构造
- desc: 执行“临近观测时空对齐与样本构造”，统一机场温湿风、能见度历史、NWP或CAMS预报的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 联合预测能见度和雾概率
- desc: 按冻结配置执行“联合预测能见度和雾概率”，完成从机场温湿风、能见度历史、NWP或CAMS预报到雾概率、能见度值与等级的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RUN_CONFIG}、{EVENT_THRESHOLDS}、{ENSEMBLE_SIZE}执行联合预测能见度和雾概率，将机场温湿风、能见度历史、NWP或CAMS预报转换为雾概率、能见度值与等级。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['联合预测能见度和雾概率结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '每个目标提前量均生成且未使用未来观测']

### s05 校准分类阈值与概率
- desc: 执行“校准分类阈值与概率”，恢复物理量、坐标和元数据，生成雾概率、能见度值与等级及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成校准分类阈值与概率，对核心结果执行已登记的校准、订正、派生或聚合并输出雾概率、能见度值与等级。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['雾概率、能见度值与等级', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '每个目标提前量均生成且未使用未来观测']

### s06 检验CSI、AUC、MAE和可靠度
- desc: 执行“检验CSI、AUC、MAE和可靠度”，使用未参与参数选择的参考资料检验雾概率、能见度值与等级，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验CSI、AUC、MAE和可靠度。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
