# 工作流：climate-hazard-factor-event-label-and-boundary-precheck-hazard-factor-workflow-4
...
- desc: 界定“多时间窗降雨驱动的区域浅层滑坡概率临近预警”的任务范围并执行“致灾因子和事件标签及任务边界预检”，核验小时雨量历史、降雨预报、滑坡目录、易发性图的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“多时间窗降雨驱动的区域浅层滑坡概率临近预警”，读取{HAZARD_DRIVERS}、{STATIC_CONDITIONS}、{EVENT_LABELS}、{TARGET_WINDOW}、{DATA_CUTOFF_TIME}并完成致灾因子和事件标签及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
- desc: 执行“致灾因子和事件标签时空对齐与样本构造”，统一小时雨量历史、降雨预报、滑坡目录、易发性图的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 生成多窗口发生概率
- desc: 按冻结配置执行“生成多窗口发生概率”，完成从小时雨量历史、降雨预报、滑坡目录、易发性图到区域滑坡概率与告警的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RUN_CONFIG}、{WARNING_THRESHOLDS}、{UNCERTAINTY_CONFIG}执行生成多窗口发生概率，将小时雨量历史、降雨预报、滑坡目录、易发性图转换为区域滑坡概率与告警。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['生成多窗口发生概率结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '预警阈值在独立验证集上冻结后评估']

### s05 结果恢复与场景产品生成
- desc: 执行“结果恢复与场景产品生成”，恢复物理量、坐标和元数据，生成区域滑坡概率与告警及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成结果恢复与场景产品生成，对核心结果执行已登记的校准、订正、派生或聚合并输出区域滑坡概率与告警。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['区域滑坡概率与告警', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '预警阈值在独立验证集上冻结后评估']

### s06 独立检验与交付判定
- desc: 执行“独立检验与交付判定”，使用未参与参数选择的参考资料检验区域滑坡概率与告警，给出适用范围与交付判定。
- depend: ['s05']
