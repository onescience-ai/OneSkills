# 工作流：climate-target-sequence-gap-and-boundary-precheck-target-sequence-gap-workflow
...
### s01 目标序列及缺口及任务边界预检
- desc: 界定“AVHRR—MODIS连续NDVI历史序列融合重建”的任务范围并执行“目标序列及缺口及任务边界预检”，核验重叠期AVHRR与MODIS NDVI、质量标志和时空特征的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“AVHRR—MODIS连续NDVI历史序列融合重建”，读取{INCOMPLETE_TARGET_DATA}、{AUXILIARY_DATA}、{GAP_MASK}、{TARGET_REGION_TIME}、{DATA_CUTOFF_TIME}并完成目标序列及缺口及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
- desc: 执行“目标序列及缺口时空对齐与样本构造”，统一重叠期AVHRR与MODIS NDVI、质量标志和时空特征的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 回推历史NDVI并填补传感器断点
- desc: 按冻结配置执行“回推历史NDVI并填补传感器断点”，完成从重叠期AVHRR与MODIS NDVI、质量标志和时空特征到校准后的连续长期NDVI序列的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RECONSTRUCTION_CONFIG}、{OUTPUT_RESOLUTION}、{UNCERTAINTY_CONFIG}执行回推历史NDVI并填补传感器断点，将重叠期AVHRR与MODIS NDVI、质量标志和时空特征转换为校准后的连续长期NDVI序列。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['回推历史NDVI并填补传感器断点结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '观测位置保持原值且缺口结果无拼接突变']

### s05 输出连续序列和观测来源标志
- desc: 执行“输出连续序列和观测来源标志”，恢复物理量、坐标和元数据，生成校准后的连续长期NDVI序列及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出连续序列和观测来源标志，对核心结果执行已登记的校准、订正、派生或聚合并输出校准后的连续长期NDVI序列。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['校准后的连续长期NDVI序列', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '观测位置保持原值且缺口结果无拼接突变']

### s06 检验重叠期偏差、趋势与突变保持
- desc: 执行“检验重叠期偏差、趋势与突变保持”，使用未参与参数选择的参考资料检验校准后的连续长期NDVI序列，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验重叠期偏差、趋势与突变保持。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
