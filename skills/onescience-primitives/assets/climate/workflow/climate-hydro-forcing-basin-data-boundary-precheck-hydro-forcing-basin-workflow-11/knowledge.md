# 工作流：climate-hydro-forcing-basin-data-boundary-precheck-hydro-forcing-basin-workflow-11
...
- desc: 界定“降雨与管网状态驱动的合流制溢流事件预报”的任务范围并执行“水文强迫和流域资料及任务边界预检”，核验降雨预报、管网水位流量、泵闸运行的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“降雨与管网状态驱动的合流制溢流事件预报”，读取{METEOROLOGICAL_FORCING}、{BASIN_ATTRIBUTES}、{HYDROLOGICAL_OBSERVATIONS}、{SIMULATION_PERIOD}、{DATA_CUTOFF_TIME}并完成水文强迫和流域资料及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
- desc: 执行“水文强迫和流域资料时空对齐与样本构造”，统一降雨预报、管网水位流量、泵闸运行的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 预测溢流概率与体量
- desc: 按冻结配置执行“预测溢流概率与体量”，完成从降雨预报、管网水位流量、泵闸运行到溢流概率、开始时间和体量的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RUN_CONFIG}、{INITIAL_STATE}、{ENSEMBLE_SIZE}执行预测溢流概率与体量，将降雨预报、管网水位流量、泵闸运行转换为溢流概率、开始时间和体量。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['预测溢流概率与体量结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '水量、状态范围和洪峰时序不存在非物理异常']

### s05 形成设施级告警
- desc: 执行“形成设施级告警”，恢复物理量、坐标和元数据，生成溢流概率、开始时间和体量及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成形成设施级告警，对核心结果执行已登记的校准、订正、派生或聚合并输出溢流概率、开始时间和体量。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['溢流概率、开始时间和体量', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '水量、状态范围和洪峰时序不存在非物理异常']

### s06 按事件漏报和体量误差验证
- desc: 执行“按事件漏报和体量误差验证”，使用未参与参数选择的参考资料检验溢流概率、开始时间和体量，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按事件漏报和体量误差验证。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
