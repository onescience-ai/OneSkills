# 工作流：climate-multi-resolution-data-and-boundary-precheck-multi-resolution-workflow-3
...
### s01 粗细分辨率资料及任务边界预检
- desc: 界定“中国0.05°历史—情景叶面积指数空间细化”的任务范围并执行“粗细分辨率资料及任务边界预检”，核验粗分辨率LAI、气候情景、土地覆盖和高分辨率参考的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“中国0.05°历史—情景叶面积指数空间细化”，读取{COARSE_INPUT}、{FINE_REFERENCE}、{TARGET_GRID}、{ANALYSIS_PERIOD}、{DATA_CUTOFF_TIME}并完成粗细分辨率资料及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
- desc: 执行“粗细分辨率资料时空对齐与样本构造”，统一粗分辨率LAI、气候情景、土地覆盖和高分辨率参考的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 学习尺度映射并批量生成细化LAI
- desc: 按冻结配置执行“学习尺度映射并批量生成细化LAI”，完成从粗分辨率LAI、气候情景、土地覆盖和高分辨率参考到0.05°历史及情景LAI序列的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RUN_CONFIG}、{ENSEMBLE_SIZE}、{OUTPUT_INTERVAL}执行学习尺度映射并批量生成细化LAI，将粗分辨率LAI、气候情景、土地覆盖和高分辨率参考转换为0.05°历史及情景LAI序列。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['学习尺度映射并批量生成细化LAI结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '输出均值、极端尾部和空间频谱均可检查']

### s05 发布分情景数据、质量图层及版本说明
- desc: 执行“发布分情景数据、质量图层及版本说明”，恢复物理量、坐标和元数据，生成0.05°历史及情景LAI序列及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成发布分情景数据、质量图层及版本说明，对核心结果执行已登记的校准、订正、派生或聚合并输出0.05°历史及情景LAI序列。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['0.05°历史及情景LAI序列', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '输出均值、极端尾部和空间频谱均可检查']

### s06 检查长期趋势、季节循环和物理范围
- desc: 执行“检查长期趋势、季节循环和物理范围”，使用未参与参数选择的参考资料检验0.05°历史及情景LAI序列，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检查长期趋势、季节循环和物理范围。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
