# 工作流：climate-pollution-met-data-boundary-precheck-pollution-met-stalign-workflow-4
...
### s01 污染气象资料及任务边界预检
- desc: 界定“近地面臭氧小时级多时效预报”的任务范围并执行“污染气象资料及任务边界预检”，核验臭氧监测历史、气象预报、可选CMAQ场的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“近地面臭氧小时级多时效预报”，读取{AIR_QUALITY_DATA}、{METEOROLOGICAL_DATA}、{EMISSION_DATA}、{TARGET_REGION_TIME}、{DATA_CUTOFF_TIME}并完成污染气象资料及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
### s02 污染气象资料时空对齐与样本构造
- desc: 执行“污染气象资料时空对齐与样本构造”，统一臭氧监测历史、气象预报、可选CMAQ场的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 逐时效预测臭氧浓度
- desc: 按冻结配置执行“逐时效预测臭氧浓度”，完成从臭氧监测历史、气象预报、可选CMAQ场到小时臭氧浓度、MDA8和超标概率的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RUN_CONFIG}、{FORECAST_HORIZON}、{SPATIAL_GRID}执行逐时效预测臭氧浓度，将臭氧监测历史、气象预报、可选CMAQ场转换为小时臭氧浓度、MDA8和超标概率。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['逐时效预测臭氧浓度结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '输出浓度或柱含量满足物理范围和掩膜约束']

### s05 计算MDA8并校准超标概率
- desc: 执行“计算MDA8并校准超标概率”，恢复物理量、坐标和元数据，生成小时臭氧浓度、MDA8和超标概率及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成计算MDA8并校准超标概率，对核心结果执行已登记的校准、订正、派生或聚合并输出小时臭氧浓度、MDA8和超标概率。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['小时臭氧浓度、MDA8和超标概率', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '输出浓度或柱含量满足物理范围和掩膜约束']

### s06 检验RMSE、峰值和事件命中率
- desc: 执行“检验RMSE、峰值和事件命中率”，使用未参与参数选择的参考资料检验小时臭氧浓度、MDA8和超标概率，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验RMSE、峰值和事件命中率。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
