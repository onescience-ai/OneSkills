# 工作流：climate-target-sequence-gap-and-boundary-precheck-target-sequence-gap-workflow-2
...
### s01 目标序列及缺口及任务边界预检
- desc: 界定“GRACE缺测期全球陆地水储量异常重建”的任务范围并执行“目标序列及缺口及任务边界预检”，核验GRACE观测、气候驱动、陆面状态的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“GRACE缺测期全球陆地水储量异常重建”，读取{GRACE_TWSA_SERIES}、{CLIMATE_LAND_DRIVERS}、{GAP_MASK}、{REFERENCE_PERIOD}、{TARGET_GRID}并完成目标序列及缺口及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
- quality_gate: ['所有必需输入均存在且路径、版本和来源可追溯', '任务区域、时段、变量和输出目标无歧义', '缺测、重复和异常资料已记录且未擅自补造', '目标序列、辅助资料和真实缺口掩膜可追溯', 'GRACE水储量序列、异常基准期和真实缺口明确']

### s02 目标序列及缺口时空对齐与样本构造
- desc: 执行“目标序列及缺口时空对齐与样本构造”，统一GRACE观测、气候驱动、陆面状态的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 重建时间缺口和历史序列
- desc: 按冻结配置执行“重建时间缺口和历史序列”，完成从GRACE观测、气候驱动、陆面状态到连续陆地水储量异常及不确定性的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RECONSTRUCTION_CONFIG}、{OBSERVED_VALUE_POLICY}、{UNCERTAINTY_CONFIG}执行重建时间缺口和历史序列，将GRACE观测、气候驱动、陆面状态转换为连续陆地水储量异常及不确定性。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['重建时间缺口和历史序列结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '观测位置保持原值且缺口结果无拼接突变', '有观测月份保持原值且仅填补指定缺口']

### s05 结果恢复与场景产品生成
- desc: 执行“结果恢复与场景产品生成”，恢复物理量、坐标和元数据，生成连续陆地水储量异常及不确定性及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成结果恢复与场景产品生成，对核心结果执行已登记的校准、订正、派生或聚合并输出连续陆地水储量异常及不确定性。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['连续陆地水储量异常及不确定性', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '观测位置保持原值且缺口结果无拼接突变']

### s06 交叉验证空间和年代泛化
- desc: 执行“交叉验证空间和年代泛化”，使用未参与参数选择的参考资料检验连续陆地水储量异常及不确定性，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成交叉验证空间和年代泛化。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
...
- quality_gate: ['验证资料未参与训练、参数选择或阈值调优', '结果与基线采用相同样本、网格和指标协议', '未达到预先登记门限时不得判定性能通过', '配置、日志、产物路径和文件哈希可追溯', '按真实缺口、伪缺口和独立时空区域分层检验', '不将陆地水储量异常直接解释为地下水储量']
