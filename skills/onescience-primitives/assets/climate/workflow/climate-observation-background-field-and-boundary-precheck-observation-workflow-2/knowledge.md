# 工作流：climate-observation-background-field-and-boundary-precheck-observation-workflow-2
...
### s01 观测和背景场及任务边界预检
- desc: 界定“稀疏观测三维温盐场快速重建与同化初始化”的任务范围并执行“观测和背景场及任务边界预检”，核验稀疏温盐剖面、卫星表层观测和背景场的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“稀疏观测三维温盐场快速重建与同化初始化”，读取{OBSERVATION_DATA}、{BACKGROUND_STATE}、{ANALYSIS_TIME}、{ASSIMILATION_WINDOW}、{DATA_CUTOFF_TIME}并完成观测和背景场及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
### s02 观测和背景场时空对齐与样本构造
- desc: 执行“观测和背景场时空对齐与样本构造”，统一稀疏温盐剖面、卫星表层观测和背景场的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 重建三维温盐场并融合多尺度背景信息
- desc: 按冻结配置执行“重建三维温盐场并融合多尺度背景信息”，完成从稀疏温盐剖面、卫星表层观测和背景场到规则网格三维温盐分析场的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{CYCLE_CONFIG}、{FORECAST_HORIZON}、{CHECKPOINT_INTERVAL}执行重建三维温盐场并融合多尺度背景信息，将稀疏温盐剖面、卫星表层观测和背景场转换为规则网格三维温盐分析场。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['重建三维温盐场并融合多尺度背景信息结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '分析增量和循环状态不存在未解释的突变']

### s05 输出分析场、不确定性和观测增量
- desc: 执行“输出分析场、不确定性和观测增量”，恢复物理量、坐标和元数据，生成规则网格三维温盐分析场及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出分析场、不确定性和观测增量，对核心结果执行已登记的校准、订正、派生或聚合并输出规则网格三维温盐分析场。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['规则网格三维温盐分析场', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '分析增量和循环状态不存在未解释的突变']

### s06 用留出剖面及同化试验检验初始化价值
- desc: 执行“用留出剖面及同化试验检验初始化价值”，使用未参与参数选择的参考资料检验规则网格三维温盐分析场，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成用留出剖面及同化试验检验初始化价值。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
...
- quality_gate: ['验证资料未参与训练、参数选择或阈值调优', '结果与基线采用相同样本、网格和指标协议', '未达到预先登记门限时不得判定性能通过', '配置、日志、产物路径和文件哈希可追溯', '分析用独立观测和后续预报两条路径验证']
