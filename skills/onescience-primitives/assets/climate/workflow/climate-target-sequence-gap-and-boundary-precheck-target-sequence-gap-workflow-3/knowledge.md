# 工作流：climate-target-sequence-gap-and-boundary-precheck-target-sequence-gap-workflow-3
...
### s01 目标序列及缺口及任务边界预检
- desc: 界定“云遮卫星海表温度缺测重建与像元不确定性估计”的任务范围并执行“目标序列及缺口及任务边界预检”，核验带云缺测的卫星海温序列、质量标志和时空坐标的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“云遮卫星海表温度缺测重建与像元不确定性估计”，读取{INCOMPLETE_TARGET_DATA}、{AUXILIARY_DATA}、{GAP_MASK}、{TARGET_REGION_TIME}、{DATA_CUTOFF_TIME}并完成目标序列及缺口及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
- desc: 执行“目标序列及缺口时空对齐与样本构造”，统一带云缺测的卫星海温序列、质量标志和时空坐标的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 恢复缺测海温并估计逐像元条件误差
- desc: 按冻结配置执行“恢复缺测海温并估计逐像元条件误差”，完成从带云缺测的卫星海温序列、质量标志和时空坐标到完整海温场及像元误差的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RECONSTRUCTION_CONFIG}、{OUTPUT_RESOLUTION}、{UNCERTAINTY_CONFIG}执行恢复缺测海温并估计逐像元条件误差，将带云缺测的卫星海温序列、质量标志和时空坐标转换为完整海温场及像元误差。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['恢复缺测海温并估计逐像元条件误差结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '观测位置保持原值且缺口结果无拼接突变']

### s05 结果恢复与场景产品生成
- desc: 执行“结果恢复与场景产品生成”，恢复物理量、坐标和元数据，生成完整海温场及像元误差及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成结果恢复与场景产品生成，对核心结果执行已登记的校准、订正、派生或聚合并输出完整海温场及像元误差。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['完整海温场及像元误差', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '观测位置保持原值且缺口结果无拼接突变']

### s06 用隐藏像元和现场观测检验误差校准
- desc: 执行“用隐藏像元和现场观测检验误差校准”，使用未参与参数选择的参考资料检验完整海温场及像元误差，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成用隐藏像元和现场观测检验误差校准。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
