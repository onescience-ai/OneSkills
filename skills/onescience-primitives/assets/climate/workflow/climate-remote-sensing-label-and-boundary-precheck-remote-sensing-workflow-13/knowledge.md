# 工作流：climate-remote-sensing-label-and-boundary-precheck-remote-sensing-workflow-13
...
### s01 遥感观测标签及任务边界预检
- desc: 界定“被动微波卫星瞬时降水率反演”的任务范围并执行“遥感观测标签及任务边界预检”，核验多通道微波亮温、扫描几何和环境辅助量的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“被动微波卫星瞬时降水率反演”，读取{SENSOR_DATA}、{AUXILIARY_DATA}、{TARGET_REGION_TIME}、{REFERENCE_LABELS}、{QUALITY_MASK}并完成遥感观测标签及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
### s02 遥感观测标签时空对齐与样本构造
- desc: 执行“遥感观测标签时空对齐与样本构造”，统一多通道微波亮温、扫描几何和环境辅助量的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 先识别降水再反演雨雪率
- desc: 按冻结配置执行“先识别降水再反演雨雪率”，完成从多通道微波亮温、扫描几何和环境辅助量到瞬时降水类别及地表降水率的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{RUN_CONFIG}、{DECISION_THRESHOLD}、{UNCERTAINTY_CONFIG}执行先识别降水再反演雨雪率，将多通道微波亮温、扫描几何和环境辅助量转换为瞬时降水类别及地表降水率。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['先识别降水再反演雨雪率结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '输出坐标、分辨率、无效值和置信信息完整']

### s05 输出降水率、类型、质量标志和不确定性
- desc: 执行“输出降水率、类型、质量标志和不确定性”，恢复物理量、坐标和元数据，生成瞬时降水类别及地表降水率及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出降水率、类型、质量标志和不确定性，对核心结果执行已登记的校准、订正、派生或聚合并输出瞬时降水类别及地表降水率。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['瞬时降水类别及地表降水率', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '输出坐标、分辨率、无效值和置信信息完整']

### s06 按海陆、强度、天气型和传感器独立验证
- desc: 执行“按海陆、强度、天气型和传感器独立验证”，使用未参与参数选择的参考资料检验瞬时降水类别及地表降水率，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按海陆、强度、天气型和传感器独立验证。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
