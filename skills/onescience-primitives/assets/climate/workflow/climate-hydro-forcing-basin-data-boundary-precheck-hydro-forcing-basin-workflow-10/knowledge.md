# 工作流：climate-hydro-forcing-basin-data-boundary-precheck-hydro-forcing-basin-workflow-10
...
- desc: 界定“遥感土壤湿度公里级空间降尺度与时序重建”的任务范围并执行“水文强迫和流域资料及任务边界预检”，核验卫星土壤湿度、植被地表温度、地形土壤属性的来源、覆盖、有效时间和可用边界。
- depend: []
- prompt: 面向“遥感土壤湿度公里级空间降尺度与时序重建”，读取{COARSE_SOIL_MOISTURE}、{FINE_SCALE_COVARIATES}、{FINE_REFERENCE}、{GAP_MASK}、{TARGET_GRID}并完成水文强迫和流域资料及任务边界预检。逐项列出数据路径、版本、区域、时段、变量、单位、缺测和资料截止时间；任何必需资料不存在、晚于截止时间或与任务边界冲突时停止并标记BLOCKED。
- step_input:
...
- quality_gate: ['所有必需输入均存在且路径、版本和来源可追溯', '任务区域、时段、变量和输出目标无歧义', '缺测、重复和异常资料已记录且未擅自补造', '气象强迫、流域边界和水文观测时空一致', '粗网格土壤湿度、细尺度参考和缺口掩膜分别登记']

### s02 水文强迫和流域资料时空对齐与样本构造
- desc: 执行“水文强迫和流域资料时空对齐与样本构造”，统一卫星土壤湿度、植被地表温度、地形土壤属性的时间、空间、单位、质量标志和缺测处理，形成可复现输入。
- depend: ['s01']
...
### s04 生成连续高分辨率场
- desc: 按冻结配置执行“生成连续高分辨率场”，完成从卫星土壤湿度、植被地表温度、地形土壤属性到高分辨率土壤湿度与质量标志的核心计算并保存逐阶段日志。
- depend: ['s03']
- prompt: 依据{DOWNSCALING_CONFIG}、{TEMPORAL_FILL_POLICY}、{UNCERTAINTY_CONFIG}执行生成连续高分辨率场，将卫星土壤湿度、植被地表温度、地形土壤属性转换为高分辨率土壤湿度与质量标志。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
- step_input:
...
- outputs: ['生成连续高分辨率场结果', '中间状态与运行日志', '资源和退出状态记录']
- quality_gate: ['目标区域和时段内结果完整且无重复或错位', '核心计算未读取任务截止时间之后的数据', '配置、随机种子、日志和中间状态能够追溯', '水量、状态范围和洪峰时序不存在非物理异常', '空间降尺度与时间缺口重建结果可分别追溯']

### s05 输出不确定性和质量掩膜
- desc: 执行“输出不确定性和质量掩膜”，恢复物理量、坐标和元数据，生成高分辨率土壤湿度与质量标志及必要的质量标志。
- depend: ['s04']
- prompt: 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出不确定性和质量掩膜，对核心结果执行已登记的校准、订正、派生或聚合并输出高分辨率土壤湿度与质量标志。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
- step_input:
...
- outputs: ['高分辨率土壤湿度与质量标志', '质量标志与不确定性信息', '产品元数据与文件清单']
- quality_gate: ['输出文件能够重新打开且变量和元数据完整', '结果单位、坐标、有效时间和物理范围已核验', '派生产品均有明确公式、源变量和质量标志', '水量、状态范围和洪峰时序不存在非物理异常']

### s06 执行站点与空间外验证
- desc: 执行“执行站点与空间外验证”，使用未参与参数选择的参考资料检验高分辨率土壤湿度与质量标志，给出适用范围与交付判定。
- depend: ['s05']
- prompt: 依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成执行站点与空间外验证。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
- step_input:
...
- quality_gate: ['验证资料未参与训练、参数选择或阈值调优', '结果与基线采用相同样本、网格和指标协议', '未达到预先登记门限时不得判定性能通过', '配置、日志、产物路径和文件哈希可追溯', '综合径流、高低流和洪峰指标分层报告', '站点留出、区域留出和伪缺口检验分别报告']
