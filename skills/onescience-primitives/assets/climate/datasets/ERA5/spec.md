## content_principle
利用 ERA5 再分析把观测和数值天气预报同化为全球规则网格多变量大气状态，用于天气预测模型训练、时序外推和气象基础模型构建。

## data_schema
ERA5 数据集位于环境变量 `$ONESCIENCE_DATASETS_DIR` 指定的路径下，目录结构如下：
...
`data/` 按年组织，每个 `YYYY.h5` 文件包含该年份全部 6 小时时间步。单个年度文件内部包含以下数据集：
...
`static/` 保存与时间无关的静态场，空间维度与 `fields` 的 `H, W` 一致；`stats/` 保存全局统计和逐变量统计文件。
...
- 通道顺序以 `fields.attrs["variables"]` 为准。
...
## storage_format
主时序数据为 HDF5 `.h5`；静态场为 NetCDF `.nc` 或 NumPy `.npy`；统计量为 NumPy `.npy`。
...
## coverage_spec
数据覆盖全球规则网格上的多变量大气状态，包含地表变量、诊断变量和多个压力层变量。当前数据以 6 小时 UTC 时次组织，闰年在当前版本中仍按 365 天、1460 个时间步处理。

## label_spec
未来时刻的大气状态可作为监督标签；历史多时刻状态和静态场可作为输入。时间索引按 6 小时递增：`t=0` 对应该年 1 月 1 日 00:00 UTC，`t=1` 对应 06:00 UTC，`t=2` 对应 12:00 UTC，`t=3` 对应 18:00 UTC。

## split_strategy
推荐按年份做因果划分，例如早期年份训练、近年验证/测试；不得随机打散时间步，以避免时间泄漏。