...
```sh
python -c "from onescience.modules.layer.generation.noise_projector import Noise_Projector; import inspect; print(inspect.signature(Noise_Projector))"
```
...
# runtime_interfaces
- `Noise_Projector`：实例化后通过 `forward` 等运行时接口参与流水线。
