# component_info
`noise_projector` 属于 `layer` 模块族，核心实现类/函数包括 `Noise_Projector`、`ProjBlock`。它的定位是网络层，由上层 OneScience 模型或流水线通过 Python API 调用。
...
# parameters
- `Noise_Projector` 构造参数：`input_length`, `configs`
