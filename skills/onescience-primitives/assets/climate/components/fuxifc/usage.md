...
```sh
python -c "from onescience.modules.fc.fuxifc import FuxiFC; import inspect; print(inspect.signature(FuxiFC))"
```
...
- `FuxiFC` 构造默认参数：`in_channels=1536`，`out_channels=70 * 4 * 4`

# runtime_interfaces
- `FuxiFC`：实例化后通过 `forward` 等运行时接口参与流水线。
