...
```sh
python -c "from onescience.modules.layer.evolution.evolution_network import Evolution_Network; import inspect; print(inspect.signature(Evolution_Network))"
```
...
- `Evolution_Network` 构造默认参数：`base_c=64`，`bilinear=True`

# runtime_interfaces
- `Evolution_Network`：实例化后通过 `forward` 等运行时接口参与流水线。
