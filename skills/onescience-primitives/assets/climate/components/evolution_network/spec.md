# component_info
`evolution_network` 属于 `layer` 模块族，核心实现类/函数包括 `Evolution_Network`。它的定位是网络层，由上层 OneScience 模型或流水线通过 Python API 调用。
...
# parameters
- `Evolution_Network` 构造参数：`n_channels`, `n_classes`, `base_c`, `bilinear`
