...
```sh
python -c "from onescience.modules.layer.generation.generative_network import Generative_Encoder; import inspect; print(inspect.signature(Generative_Encoder))"
```
...
- `Generative_Encoder` 构造默认参数：`base_c=64`

# runtime_interfaces
- `Generative_Encoder`：实例化后通过 `forward` 等运行时接口参与流水线。
- `Generative_Decoder`：实例化后通过 `forward` 等运行时接口参与流水线。
