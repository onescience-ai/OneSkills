# component_info
`generative_network` 属于 `layer` 模块族，核心实现类/函数包括 `Generative_Encoder`、`Generative_Decoder`。它的定位是网络层，由上层 OneScience 模型或流水线通过 Python API 调用。
...
# parameters
- `Generative_Encoder` 构造参数：`n_channels`, `base_c`
- `Generative_Decoder` 构造参数：`opt`
