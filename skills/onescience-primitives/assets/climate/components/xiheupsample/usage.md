...
```sh
python -c "from onescience.modules.sample.xiheupsample import XiheUpSample; import inspect; print(inspect.signature(XiheUpSample))"
```
...
# runtime_interfaces
- `XiheUpSample`：实例化后通过 `forward` 等运行时接口参与流水线。
