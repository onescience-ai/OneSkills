# 实例任务：检验命中率、虚警率和可靠度 @ E52

- domain: climate
- 骨架: climate-hit-false-alarm-reliability-task
- 场景: climate-ground-meteorological-driven-0to30min-lightning-nowcast-scenario (E52)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“检验命中率、虚警率和可靠度”，使用未参与参数选择的参考资料检验分时间窗雷电发生概率和告警，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验命中率、虚警率和可靠度。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
