...
- domain: climate
- 骨架: climate-validation-by-unseen-event-arrival-time-task
- 场景: climate-tsunami-geodetic-observation-driven-rapid-inundation-range-scenario (E90)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“按未见事件及到时验证”，使用未参与参数选择的参考资料检验沿岸最大水深和淹没范围，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按未见事件及到时验证。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
