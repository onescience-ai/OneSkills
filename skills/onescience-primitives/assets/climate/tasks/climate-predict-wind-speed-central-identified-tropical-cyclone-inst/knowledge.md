# 实例任务：预测风速和中心气压变化 @ E50

- domain: climate
- 骨架: climate-predict-wind-speed-central-pressure-change-task
- 场景: climate-identified-tropical-cyclone-intensity-forecast-12-72h-scenario (E50)
- step_id: s04
- depend: ['s03']
...
## 本实例步骤描述
按冻结配置执行“预测风速和中心气压变化”，完成从气旋强度历史、海温、环境大气场到多时效最大持续风速、最低气压与强度等级的核心计算并保存逐阶段日志。

## 本实例执行 prompt
依据{RUN_CONFIG}、{OUTPUT_INTERVAL}、{ENSEMBLE_SIZE}执行预测风速和中心气压变化，将气旋强度历史、海温、环境大气场转换为多时效最大持续风速、最低气压与强度等级。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
