# 实例任务：分别验证分析误差与预报技巧 @ E4

- domain: climate
- 骨架: climate-analysis-error-forecast-skill-separate-validation-task
...
## 本实例步骤描述
执行“分别验证分析误差与预报技巧”，使用未参与参数选择的参考资料检验全球分析场与后续多变量天气预报，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成分别验证分析误差与预报技巧。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
