# 实例任务：按年份检验区分度、校准度和可解释性 @ E37

- domain: climate
- 骨架: climate-testing-discrimination-calibration-interpretability-by-year-task
- 场景: climate-weather-driven-next-day-wildfire-danger-probability-scenario (E37)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“按年份检验区分度、校准度和可解释性”，使用未参与参数选择的参考资料检验次日大火概率及危险等级图，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按年份检验区分度、校准度和可解释性。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
