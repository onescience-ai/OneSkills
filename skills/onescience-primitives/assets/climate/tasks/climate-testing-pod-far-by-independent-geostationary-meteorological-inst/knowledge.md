# 实例任务：按独立过程检验POD和FAR @ E80

- domain: climate
- 骨架: climate-testing-pod-far-by-independent-process-task
- 场景: climate-geostationary-meteorological-satellite-driven-convection-scenario (E80)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“按独立过程检验POD和FAR”，使用未参与参数选择的参考资料检验初生概率格点、对流对象和时间标记，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按独立过程检验POD和FAR。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
