# 实例任务：检验谱结构、极值和集合覆盖率 @ E55

- domain: climate
- 骨架: climate-spectrum-extreme-ensemble-coverage-task
...
## 本实例步骤描述
执行“检验谱结构、极值和集合覆盖率”，使用未参与参数选择的参考资料检验公里尺度多变量确定性场或集合细场，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验谱结构、极值和集合覆盖率。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
