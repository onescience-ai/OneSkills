...
- domain: climate
- 骨架: climate-object-level-graded-alert-task
...
## 本实例步骤描述
执行“生成对象级分级告警”，恢复物理量、坐标和元数据，生成阵风概率、最大风速和风险对象及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成生成对象级分级告警，对核心结果执行已登记的校准、订正、派生或聚合并输出阵风概率、最大风速和风险对象。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
