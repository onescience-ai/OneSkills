...
## 步骤描述（跨场景聚合去重）
- 执行“订正晴空指数偏差”，恢复物理量、坐标和元数据，生成多时效辐照度场或站点序列及必要的质量标志。

## 执行 prompt（跨场景聚合去重）
- 依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成订正晴空指数偏差，对核心结果执行已登记的校准、订正、派生或聚合并输出多时效辐照度场或站点序列。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
...
## 实例任务（本骨架在各场景的实例化）
- climate-correct-clear-sky-index-bias-sky-camera-satellite-solar-inst
