...
- domain: climate
- 骨架: climate-physical-consistency-check-task
- 场景: climate-cmip6-climate-scenario-watershed-runoff-projection-scenario (E28)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“检验物理一致性”，使用未参与参数选择的参考资料检验未来径流序列、变化率与极端统计，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验物理一致性。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
