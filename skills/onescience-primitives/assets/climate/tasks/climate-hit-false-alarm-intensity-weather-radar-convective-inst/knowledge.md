# 实例任务：检验命中率、虚警率和强度误差 @ E85

- domain: climate
- 骨架: climate-hit-false-alarm-intensity-error-task
- 场景: climate-weather-radar-convective-gust-nowcast-scenario (E85)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“检验命中率、虚警率和强度误差”，使用未参与参数选择的参考资料检验阵风概率、最大风速和风险对象，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验命中率、虚警率和强度误差。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
