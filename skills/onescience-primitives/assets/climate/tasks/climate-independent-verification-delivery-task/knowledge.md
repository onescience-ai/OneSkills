...
## 步骤描述（跨场景聚合去重）
- 执行“独立检验与交付判定”，使用未参与参数选择的参考资料检验云概率、类别及质量掩膜，给出适用范围与交付判定。
- 执行“独立检验与交付判定”，使用未参与参数选择的参考资料检验区域滑坡概率与告警，给出适用范围与交付判定。
- 执行“独立检验与交付判定”，使用未参与参数选择的参考资料检验多时效流量、洪峰与峰时，给出适用范围与交付判定。
...
## 实例任务（本骨架在各场景的实例化）
- climate-independent-verification-multi-time-window-rainfall-inst
- climate-independent-verification-visible-image-cloud-mask-inst
- climate-independent-verification-weather-forecast-driven-inst
