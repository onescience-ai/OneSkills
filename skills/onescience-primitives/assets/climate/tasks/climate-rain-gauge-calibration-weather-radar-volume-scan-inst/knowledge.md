# 实例任务：应用雨量计校准并生成雨强产品 @ E34

- domain: climate
- 骨架: climate-rain-gauge-calibration-rainfall-intensity-product-task
...
## 本实例步骤描述
执行“应用雨量计校准并生成雨强产品”，恢复物理量、坐标和元数据，生成当前雨强格点、站点降水与质量掩膜及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成应用雨量计校准并生成雨强产品，对核心结果执行已登记的校准、订正、派生或聚合并输出当前雨强格点、站点降水与质量掩膜。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
