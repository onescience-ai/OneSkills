# 实例任务：订正系统性位相与振幅偏差 @ E19

- domain: climate
- 骨架: climate-correct-systematic-phase-and-amplitude-bias-task
- 场景: climate-mjo-intraseasonal-oscillation-10-30-day-phase-forecast-scenario (E19)
- step_id: s05
- depend: ['s04']
...
## 本实例步骤描述
执行“订正系统性位相与振幅偏差”，恢复物理量、坐标和元数据，生成MJO或ISO指数、位相振幅及空间异常及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成订正系统性位相与振幅偏差，对核心结果执行已登记的校准、订正、派生或聚合并输出MJO或ISO指数、位相振幅及空间异常。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
