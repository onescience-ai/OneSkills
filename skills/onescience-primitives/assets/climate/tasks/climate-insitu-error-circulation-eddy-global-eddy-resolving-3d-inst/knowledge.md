# 实例任务：检验原位误差、环流、涡旋、频谱和漂移 @ E5

- domain: climate
- 骨架: climate-insitu-error-circulation-eddy-spectrum-drift-task
- 场景: climate-global-eddy-resolving-3d-ocean-1to10day-forecast-scenario (E5)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“检验原位误差、环流、涡旋、频谱和漂移”，使用未参与参数选择的参考资料检验多时效温盐流及海表高度场，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验原位误差、环流、涡旋、频谱和漂移。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
