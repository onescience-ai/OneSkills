# 实例任务：检验RMSE、相关性和污染事件技巧 @ E99

- domain: climate
- 骨架: climate-rmse-correlation-pollution-event-task
- 场景: climate-nwp-emission-driven-regional-aerosol-0-72h-forecast-scenario (E99)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“检验RMSE、相关性和污染事件技巧”，使用未参与参数选择的参考资料检验分时效PM2.5或气溶胶浓度格点与高污染区域，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验RMSE、相关性和污染事件技巧。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
