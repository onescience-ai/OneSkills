# 实例任务：按时效检验FSS、CSI和Brier分数 @ E8

- domain: climate
- 骨架: climate-testing-fss-csi-brier-score-by-validity-period-task
...
## 本实例步骤描述
执行“按时效检验FSS、CSI和Brier分数”，使用未参与参数选择的参考资料检验3—12小时降水率场、累积降水和概率，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按时效检验FSS、CSI和Brier分数。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
