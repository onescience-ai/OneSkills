# 实例任务：按独立测次和高变化区评价误差 @ E74

- domain: climate
- 骨架: climate-error-evaluation-by-independent-measurement-high-variability-task
...
## 本实例步骤描述
执行“按独立测次和高变化区评价误差”，使用未参与参数选择的参考资料检验完整浴深场及未来海床高程变化，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按独立测次和高变化区评价误差。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
