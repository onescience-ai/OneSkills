# 实例任务：按提前期与分析场、浮标及基线比较 @ E27

- domain: climate
- 骨架: climate-comparison-by-lead-time-with-analysis-buoys-baseline-task
...
## 本实例步骤描述
执行“按提前期与分析场、浮标及基线比较”，使用未参与参数选择的参考资料检验未来1—7日区域海表温度场，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按提前期与分析场、浮标及基线比较。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
