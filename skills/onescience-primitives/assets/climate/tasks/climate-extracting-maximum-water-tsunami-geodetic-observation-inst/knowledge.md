...
- domain: climate
- 骨架: climate-extracting-maximum-water-depth-extent-task
...
## 本实例步骤描述
执行“提取最大水深和范围”，恢复物理量、坐标和元数据，生成沿岸最大水深和淹没范围及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成提取最大水深和范围，对核心结果执行已登记的校准、订正、派生或聚合并输出沿岸最大水深和淹没范围。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
