# 实例任务：恢复物理单位并拼接区域 @ E55

- domain: climate
- 骨架: climate-restoring-physical-units-stitching-regions-task
...
## 本实例步骤描述
执行“恢复物理单位并拼接区域”，恢复物理量、坐标和元数据，生成公里尺度多变量确定性场或集合细场及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成恢复物理单位并拼接区域，对核心结果执行已登记的校准、订正、派生或聚合并输出公里尺度多变量确定性场或集合细场。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
