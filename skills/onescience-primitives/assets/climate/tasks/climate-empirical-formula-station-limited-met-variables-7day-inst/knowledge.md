# 实例任务：与经验公式及站点观测比较 @ E66

- domain: climate
- 骨架: climate-empirical-formula-station-observation-comparison-task
...
## 本实例步骤描述
执行“与经验公式及站点观测比较”，使用未参与参数选择的参考资料检验未来7天参考蒸散发序列，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成与经验公式及站点观测比较。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
