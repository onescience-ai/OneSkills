# 实例任务：恢复量纲并处理缺测站点 @ E41

- domain: climate
- 骨架: climate-restoring-dimensions-handling-missing-stations-task
...
## 本实例步骤描述
执行“恢复量纲并处理缺测站点”，恢复物理量、坐标和元数据，生成各站未来逐时温度、湿度和风速序列及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成恢复量纲并处理缺测站点，对核心结果执行已登记的校准、订正、派生或聚合并输出各站未来逐时温度、湿度和风速序列。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
