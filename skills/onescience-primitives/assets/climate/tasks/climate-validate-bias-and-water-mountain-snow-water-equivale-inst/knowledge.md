# 实例任务：用雪站和留出年份验证偏差及水量一致性 @ E40

- domain: climate
- 骨架: climate-validate-bias-and-water-balance-with-snow-stations-task
...
## 本实例步骤描述
执行“用雪站和留出年份验证偏差及水量一致性”，使用未参与参数选择的参考资料检验当前SWE格点场、流域雪储量及质量标志，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成用雪站和留出年份验证偏差及水量一致性。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
