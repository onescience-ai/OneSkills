# 实例任务：检验可靠度、区分度与跨年份稳定性 @ E70

- domain: climate
- 骨架: climate-reliability-discrimination-cross-year-stability-task
- 场景: climate-climate-fuel-anomaly-driven-seasonal-wildfire-probability-scenario (E70)
- step_id: s06
- depend: ['s05']

## 场景研究主体
- E70
- 关联论文: Seasonal Fire Prediction using Spatio-Temporal Deep Neural Networks | doi:

## 本实例步骤描述
执行“检验可靠度、区分度与跨年份稳定性”，使用未参与参数选择的参考资料检验未来火季区域发生概率和风险分级，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验可靠度、区分度与跨年份稳定性。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
