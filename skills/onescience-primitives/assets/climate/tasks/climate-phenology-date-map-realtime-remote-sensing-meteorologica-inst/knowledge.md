# 实例任务：发布物候日期图及实时使用边界 @ E67

- domain: climate
- 骨架: climate-phenology-date-map-realtime-boundary-release-task
...
## 本实例步骤描述
执行“发布物候日期图及实时使用边界”，恢复物理量、坐标和元数据，生成各物候阶段年积日及置信区间及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成发布物候日期图及实时使用边界，对核心结果执行已登记的校准、订正、派生或聚合并输出各物候阶段年积日及置信区间。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
