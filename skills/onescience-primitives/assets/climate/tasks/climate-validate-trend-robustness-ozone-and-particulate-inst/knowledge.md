# 实例任务：用重采样区间检验趋势稳健性 @ E45

- domain: climate
- 骨架: climate-validate-trend-robustness-with-resampling-intervals-task
...
## 本实例步骤描述
执行“用重采样区间检验趋势稳健性”，使用未参与参数选择的参考资料检验气象归一化浓度、趋势分解和特征贡献，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成用重采样区间检验趋势稳健性。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
...
- 未达到预先登记门限时不得判定性能通过
- 配置、日志、产物路径和文件哈希可追溯
- 效应量、置信区间、显著性和敏感性分析同时报告
- 统计关联、趋势分解和因果归因不得混作同一结论
