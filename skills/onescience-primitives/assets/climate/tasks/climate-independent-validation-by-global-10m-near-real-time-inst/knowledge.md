# 实例任务：按类别、区域和边界像元独立验证 @ E71

- domain: climate
- 骨架: climate-independent-validation-by-category-region-boundary-pixel-task
- 场景: climate-global-10m-near-real-time-land-cover-mapping-scenario (E71)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“按类别、区域和边界像元独立验证”，使用未参与参数选择的参考资料检验10米土地覆盖类别概率图，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按类别、区域和边界像元独立验证。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
