# 实例任务：输出当前与未来污染格点产品 @ E60

- domain: climate
- 骨架: climate-output-current-and-future-pollution-grid-products-task
...
## 本实例步骤描述
执行“输出当前与未来污染格点产品”，恢复物理量、坐标和元数据，生成城市连续污染格点、短时预测及不确定性及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出当前与未来污染格点产品，对核心结果执行已登记的校准、订正、派生或聚合并输出城市连续污染格点、短时预测及不确定性。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
