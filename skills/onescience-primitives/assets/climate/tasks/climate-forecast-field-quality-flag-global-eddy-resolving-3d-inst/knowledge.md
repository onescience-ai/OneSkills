# 实例任务：发布预报场、质量标志和运行配置 @ E5

- domain: climate
- 骨架: climate-forecast-field-quality-flag-configuration-release-task
...
## 本实例步骤描述
执行“发布预报场、质量标志和运行配置”，恢复物理量、坐标和元数据，生成多时效温盐流及海表高度场及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成发布预报场、质量标志和运行配置，对核心结果执行已登记的校准、订正、派生或聚合并输出多时效温盐流及海表高度场。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
