# 实例任务：检验MAE和快速增强命中情况 @ E50

- domain: climate
- 骨架: climate-mae-rapid-intensification-hit-task
...
## 本实例步骤描述
执行“检验MAE和快速增强命中情况”，使用未参与参数选择的参考资料检验多时效最大持续风速、最低气压与强度等级，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成检验MAE和快速增强命中情况。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
