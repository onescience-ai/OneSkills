# 实例任务：用隐藏像元和现场观测检验误差校准 @ E69

- domain: climate
- 骨架: climate-validate-error-calibration-with-hidden-pixels-and-field-obs-task
...
## 本实例步骤描述
执行“用隐藏像元和现场观测检验误差校准”，使用未参与参数选择的参考资料检验完整海温场及像元误差，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成用隐藏像元和现场观测检验误差校准。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
