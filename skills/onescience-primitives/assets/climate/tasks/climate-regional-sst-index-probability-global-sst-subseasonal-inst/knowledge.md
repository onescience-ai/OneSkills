# 实例任务：派生区域海温指数并校准概率 @ E29

- domain: climate
- 骨架: climate-regional-sst-index-probability-calibration-task
...
## 本实例步骤描述
执行“派生区域海温指数并校准概率”，恢复物理量、坐标和元数据，生成多月海温异常场及气候指数及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成派生区域海温指数并校准概率，对核心结果执行已登记的校准、订正、派生或聚合并输出多月海温异常场及气候指数。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
