# 实例任务：学习尺度映射并批量生成细化LAI @ E93

- domain: climate
- 骨架: climate-scale-mapping-batch-refined-lai-generation-task
- 场景: climate-china-0-05-historical-scenario-lai-spatial-downscaling-scenario (E93)
- step_id: s04
- depend: ['s03']
...
## 本实例步骤描述
按冻结配置执行“学习尺度映射并批量生成细化LAI”，完成从粗分辨率LAI、气候情景、土地覆盖和高分辨率参考到0.05°历史及情景LAI序列的核心计算并保存逐阶段日志。

## 本实例执行 prompt
依据{RUN_CONFIG}、{ENSEMBLE_SIZE}、{OUTPUT_INTERVAL}执行学习尺度映射并批量生成细化LAI，将粗分辨率LAI、气候情景、土地覆盖和高分辨率参考转换为0.05°历史及情景LAI序列。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
