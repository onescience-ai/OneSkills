# 实例任务：计算MDA8并校准超标概率 @ E44

- domain: climate
- 骨架: climate-compute-mda8-and-calibrate-exceedance-probability-task
- 场景: climate-near-surface-ozone-hourly-multi-horizon-forecast-scenario (E44)
- step_id: s05
- depend: ['s04']
...
## 本实例步骤描述
执行“计算MDA8并校准超标概率”，恢复物理量、坐标和元数据，生成小时臭氧浓度、MDA8和超标概率及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成计算MDA8并校准超标概率，对核心结果执行已登记的校准、订正、派生或聚合并输出小时臭氧浓度、MDA8和超标概率。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
