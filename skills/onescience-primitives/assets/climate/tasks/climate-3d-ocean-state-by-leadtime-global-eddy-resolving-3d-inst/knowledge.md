# 实例任务：生成各提前期三维海洋状态场 @ E5

- domain: climate
- 骨架: climate-3d-ocean-state-by-leadtime-task
...
## 本实例步骤描述
按冻结配置执行“生成各提前期三维海洋状态场”，完成从三维海洋分析场、海表温度和海面风到多时效温盐流及海表高度场的核心计算并保存逐阶段日志。

## 本实例执行 prompt
依据{RUN_CONFIG}、{OUTPUT_INTERVAL}、{ENSEMBLE_SIZE}执行生成各提前期三维海洋状态场，将三维海洋分析场、海表温度和海面风转换为多时效温盐流及海表高度场。保存配置、随机种子、开始结束时间、退出状态和中间产物；不得临时更换数据、阈值或方法版本。
