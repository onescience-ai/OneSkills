# 实例任务：汇总栅格产品并给出误差传播 @ E59

- domain: climate
- 骨架: climate-raster-product-error-propagation-task
...
## 本实例步骤描述
执行“汇总栅格产品并给出误差传播”，恢复物理量、坐标和元数据，生成森林结构参数及碳储量图及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成汇总栅格产品并给出误差传播，对核心结果执行已登记的校准、订正、派生或聚合并输出森林结构参数及碳储量图。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
