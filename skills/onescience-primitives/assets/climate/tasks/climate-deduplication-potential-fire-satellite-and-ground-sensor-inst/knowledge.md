...
- domain: climate
- 骨架: climate-deduplication-potential-fire-detection-task
- 场景: climate-satellite-and-ground-sensor-fusion-early-fire-smoke-plume-scenario (E22)
- step_id: s05
- depend: ['s04']
...
## 本实例步骤描述
执行“去重并生成疑似火点”，恢复物理量、坐标和元数据，生成火情类别、位置、置信度与告警及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成去重并生成疑似火点，对核心结果执行已登记的校准、订正、派生或聚合并输出火情类别、位置、置信度与告警。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
