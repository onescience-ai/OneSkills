# 实例任务：输出农田概率、二值范围和面积统计 @ E56

- domain: climate
- 骨架: climate-output-farmland-probability-binary-range-and-area-task
...
## 本实例步骤描述
执行“输出农田概率、二值范围和面积统计”，恢复物理量、坐标和元数据，生成30米农田范围图及区域面积及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出农田概率、二值范围和面积统计，对核心结果执行已登记的校准、订正、派生或聚合并输出30米农田范围图及区域面积。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
