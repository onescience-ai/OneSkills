# 实例任务：按云型、角度、地表和季节分层验证 @ E54

- domain: climate
- 骨架: climate-validation-by-cloud-type-angle-surface-season-stratification-task
- 场景: climate-geostationary-satellite-cirrus-coverage-and-optical-property-scenario (E54)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“按云型、角度、地表和季节分层验证”，使用未参与参数选择的参考资料检验卷云掩膜及光学属性，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按云型、角度、地表和季节分层验证。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
