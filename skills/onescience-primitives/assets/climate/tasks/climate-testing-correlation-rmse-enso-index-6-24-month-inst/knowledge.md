# 实例任务：按提前量检验相关、RMSE和可靠度 @ E9

- domain: climate
- 骨架: climate-testing-correlation-rmse-reliability-by-lead-time-task
...
## 本实例步骤描述
执行“按提前量检验相关、RMSE和可靠度”，使用未参与参数选择的参考资料检验Niño3.4指数集合、厄尔尼诺和拉尼娜概率，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成按提前量检验相关、RMSE和可靠度。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
