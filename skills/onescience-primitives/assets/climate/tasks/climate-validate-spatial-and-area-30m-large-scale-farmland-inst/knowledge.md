# 实例任务：用混淆矩阵及统计资料检验空间和面积 @ E56

- domain: climate
- 骨架: climate-validate-spatial-and-area-with-confusion-matrix-task
...
## 本实例步骤描述
执行“用混淆矩阵及统计资料检验空间和面积”，使用未参与参数选择的参考资料检验30米农田范围图及区域面积，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成用混淆矩阵及统计资料检验空间和面积。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
