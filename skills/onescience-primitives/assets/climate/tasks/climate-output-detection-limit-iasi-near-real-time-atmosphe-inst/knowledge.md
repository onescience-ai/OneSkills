# 实例任务：输出检测限、不确定性和质量等级 @ E79

- domain: climate
- 骨架: climate-output-detection-limit-uncertainty-and-quality-grade-task
...
## 本实例步骤描述
执行“输出检测限、不确定性和质量等级”，恢复物理量、坐标和元数据，生成NH3总柱浓度、误差及质量标志及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出检测限、不确定性和质量等级，对核心结果执行已登记的校准、订正、派生或聚合并输出NH3总柱浓度、误差及质量标志。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
