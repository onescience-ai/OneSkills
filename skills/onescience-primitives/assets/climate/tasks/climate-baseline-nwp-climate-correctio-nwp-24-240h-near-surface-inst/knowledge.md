# 实例任务：与原始NWP及气候订正基线比较 @ E53

- domain: climate
- 骨架: climate-baseline-nwp-climate-correction-comparison-task
- 场景: climate-nwp-24-240h-near-surface-multivariate-bias-correction-scenario (E53)
- step_id: s06
- depend: ['s05']
...
## 本实例步骤描述
执行“与原始NWP及气候订正基线比较”，使用未参与参数选择的参考资料检验同网格多时效近地面变量订正场，给出适用范围与交付判定。

## 本实例执行 prompt
依据{VERIFICATION_REFERENCE}、{BENCHMARK_RESULT}、{METRICS}、{ACCEPTANCE_CRITERIA}以独立参考资料完成与原始NWP及气候订正基线比较。按预先登记的区域、时段、事件或提前期计算指标并与同协议基线比较；缺少参考资料或验收门限时只能报告结果，不得声明性能PASS。
