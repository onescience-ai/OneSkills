# 实例任务：按冻结阈值形成区域风险分级 @ E70

- domain: climate
- 骨架: climate-forming-regional-risk-grading-freezing-threshold-task
...
## 本实例步骤描述
执行“按冻结阈值形成区域风险分级”，恢复物理量、坐标和元数据，生成未来火季区域发生概率和风险分级及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成按冻结阈值形成区域风险分级，对核心结果执行已登记的校准、订正、派生或聚合并输出未来火季区域发生概率和风险分级。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
