# 实例任务：输出逐像元不确定性和适用域标志 @ E54

- domain: climate
- 骨架: climate-output-pixelwise-uncertainty-and-domain-flag-task
...
## 本实例步骤描述
执行“输出逐像元不确定性和适用域标志”，恢复物理量、坐标和元数据，生成卷云掩膜及光学属性及必要的质量标志。

## 本实例执行 prompt
依据{OUTPUT_FORMAT}、{OUTPUT_DIRECTORY}、{POSTPROCESS_CONFIG}、{DERIVED_PRODUCTS}完成输出逐像元不确定性和适用域标志，对核心结果执行已登记的校准、订正、派生或聚合并输出卷云掩膜及光学属性。核验单位、坐标、时次、无效值、物理范围和文件可读性；派生量必须记录公式和源变量。
